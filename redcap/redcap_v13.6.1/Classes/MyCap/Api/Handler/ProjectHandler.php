...
class ProjectHandler
{
    ...
    public static function mergeConfig($config, $fields)
    {
        ...
        if (!property_exists(
            ...
            foreach ($tasks as $idx => $task) {
                ...
                if (!count($aryFormFields[$task->identifier])) {
                    // There must be at least 1 question
                    $ret_val["errors"][] = "Invalid config: Task \"" . $task->identifier . "\" has no fields. ";
                    continue;
                }
     ...
     *
     * @param string $fields REDCap fields returned from API
     * @return array
     */
     ...
     *
     * @param string $field A REDCap field object
     * @return object
     */
    private static function formatFieldForMyCap($field)
    {
        ...
        $ret_val["title"] = $field->field_label;
        $ret_val["required"] = ($field->required_field == "y") ? true : false;

        if (strlen($field->branching_logic)) {
            $ret_val["branchingLogic"] = $field->branching_logic;
        }

        if (strlen($field->section_header)) {
            $ret_val["sectionTitle"] = $field->section_header;
        }
