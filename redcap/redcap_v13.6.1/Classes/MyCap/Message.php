...
class Message
{
    ...
    public static function renderAppDesignTabs()
    {
        ...
        //TODO - MyCap::If existing config and published config are same, disable "Publish" button
        //$areConfigsSame = $myCapProj->compareProjectConfigs(PROJECT_ID);
        $disabled = '';// ($areConfigsSame == true) ? 'disabled' : '';

        $projectConfig = json_decode($myCapProj->config);

//        if ($user_rights['design']) {
//            print "<div class='mt-1 mb-3'><button class='btn btn-xs btn-defaultrc text-primaryrc fs12' onclick=\"window.location.href=app_path_webroot+'Design/online_designer.php?pid='+pid;\"><i class='fas fa-chevron-circle-left'></i> {$lang['mycap_mobile_app_634']}</button></div>";
//        }
