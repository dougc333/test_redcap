...
class MyCap
{
    ...
    {
        $projectConfig = json_decode($this->generateProjectConfigJSON(PROJECT_ID));
    ...
    /**
     * Compare old and new project configs
     *
     * @param int $projectId
     * @return boolean
     */
    public function compareProjectConfigs($projectId = null)
    {
        if (is_null($projectId)) $projectId = PROJECT_ID;

        $existingConfig = json_decode($this->config, true);
        $newConfig = json_decode($this->generateProjectConfigJSON($projectId), true);

        $result = array_diff($existingConfig, $newConfig);
    ...
    public function generateHmacKey()
    {
        do {
            // Excluding letters I & O and number 0 to avoid confusion
            $hmacKey = bin2hex($this->generateHmacRandomString(32));
     ...
     */
    public function generateHmacRandomString($length = 20) {
     ...
     * @param void
     */
    public function copyProjectParticipants($copyof_project_id, $new_project_id) {
        $sql = "SELECT * FROM redcap_mycap_participants WHERE project_id = $copyof_project_id";
        $q = db_query($sql);
        while ($row = db_fetch_assoc($q))
        {
            db_query("INSERT INTO redcap_mycap_participants (code, project_id, record, join_date, push_notification_ids, is_deleted) 
                            VALUES ('" . db_escape(Participant::generateUniqueCode($new_project_id)) . "', {$new_project_id}, '".db_escape($row['record'])."',
                                    ".checkNull($row['join_date']).", '".db_escape($row['push_notification_ids'])."', '".db_escape($row['is_deleted'])."')");
        }
    }
        ...
        return
            RCView::div(array('class' => 'clearfix mx-2'),
                RCView::div(array('class' => 'float-start pt-2 pe-4', 'style' => "width:$leftsideWidth;"),
                    RCView::img(['src' => 'mycap_screen1.png', 'class' => 'ms-4 me-5', 'style' => 'height:350px;']) .
                ...
                RCView::div(array('style' => "margin-left:$leftsideWidth;"),
                    ...
                    RCView::div(array('class' => 'mb-4 fs14 boldish py-2 px-3', 'style'=>'border:1px solid #eee;'),
                        RCView::div(array('class' => 'mb-1'), $lang['mycap_mobile_app_686']) .
                        RCView::div(array('class' => ''),
                            $lang['mycap_mobile_app_687'] .
                            RCView::a(['href' => APP_PATH_WEBROOT."Resources/misc/mycap_help.pdf", 'target' => '_blank', 'class'=>'text-dangerrc fs14 ms-2', 'style' => 'text-decoration:underline'],
