...
class SyncIssues
{
    ...
    public static function getSyncIssues($project_id = null)
    {
        ...
        // Get main attributes
        $sql = "SELECT * FROM redcap_mycap_syncissues WHERE project_code = '".$project_code."'";
    ...
    public static function getUnresolvedIssuesCount($projectId) {
        ...
        $sql = "SELECT COUNT(*) FROM redcap_mycap_syncissues WHERE project_code = '".$project_code."' AND resolved = 0";
        if (!empty($codesList)) $sql .= " AND participant_code IN (".prep_implode($codesList).")";
    ...
    public static function getSyncIssueDetails($participantCode, $issueId)
    {
        ...
        $sql = "SELECT * FROM redcap_mycap_syncissues WHERE uuid = '".$issueId."'";
        $q = db_query($sql);
        $details = array();
        while ($row = db_fetch_assoc($q)) {
            $formLink = "<a href='" . APP_PATH_WEBROOT . "Design/online_designer.php?pid=".PROJECT_ID."&page=".$row['instrument']."' style='font-weight:bold;text-decoration:underline;'>".$Proj->forms[$row['instrument']]['menu']."</a>";
    ...
    public static function displaySyncIssueDetails($projectCode, $participantCode, $issueId)
    {
        ...
        if ($participantWasLoaded) {
            //$vars['recordId'] = $participantEntity->id;
        ...
        <form id="SyncIssueSetupForm">
            ...
            <div>
                ...
                <div>
                    <table cellspacing="1" width="100%" style="1px solid black">
                        <tr>
     ...
     * @return string
     */
    public function getErrorTypeText($errorType)
    {
        global $lang;
        switch ($errorType) {
