...
class Calendar
{
	...
	*/
    ...
    public static function buildTimezones($timezone, $start, $end) {
		...
		$getTransitions = function() use($timezone, $start, $end, $allTransitions, $getPreviousTransition) {
			
			// filter based on the start and end timestamps
			$transitions = array_filter($allTransitions, function($transition) use($start, $end) {
				$time =  intval($transition['ts']);
