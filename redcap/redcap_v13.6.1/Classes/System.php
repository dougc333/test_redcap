...
class System
{
	...
	public static function getReadReplicaLagTime()
	{
		...
		return ($row['Seconds_Behind_Master'] ?? 0)*1;
	}

	// Determine if the Read Replica DB's lag time is too high to be utilized
	public static function readReplicaLagTimeIsTooHigh()
	{
	...
	public static function useReadReplicaDB()
	{
		...
		###############################################
		## SUBSET LIST FOR INITIAL TESTING ON VANDERBILT PROD
		...
		return ((
			...
			!isset($_GET['pid']) && (
				...
				// Public Project Dashboard
				|| (ends_with($uri, 'surveys/index.php') && isset($_GET['__dashboard']) && is_string($_GET['__dashboard']) && !isset($_GET['a']) && !isset($_GET['sq']) && !isset($_GET['s']) && !isset($_GET['hash']))
				// Specific API exports (data/report/logging)
				|| (defined("API") && (isset($_POST['token']) && isset($_POST['content'])
					&& (
						// Data exports
						(isset($_POST['action']) && $_POST['content'] == 'record' && $_POST['action'] == 'export')
						// Report exports
						|| (isset($_POST['report_id']) && $_POST['content'] == 'report')
						// Logging exports
						|| ($_POST['content'] == 'log')
					)
				))
	...
	public static function init()
	{
		...
		define('ALLOWED_TAGS', '<strike><style><code><video><audio><source><caption><canvas><ol><ul><li><label><pre><p><a><br><center><font><b><i><u><h6><h5><h4><h3><h2><h1><hr><table><tbody><tr><th><td><thead><tfoot><img><span><div><em><strong><acronym><sub><sup><map><area>');
		// Set error handler
	...
	public static function checkSystemStatus()
	{
		...
		if ($system_offline && defined("PAGE") && PAGE != 'ControlCenter/check.php' && PAGE != 'ControlCenter/check_server_ping.php'
            && !((defined('SUPER_USER') && SUPER_USER) || (defined('ACCESS_SYSTEM_CONFIG') && ACCESS_SYSTEM_CONFIG)))
		{
	...
	public static function checkDbEncoding($useReadReplica=false)
	{
		...
		if (!isset($db_character_set) || $db_character_set == '') {
			...
			if (!$useReadReplica && ($db_character_set == '' || $db_collation == '')) {
	...
	public static function checkQuery($sql){
		...
		else if(!preg_match('/[^[0-9a-z$_]redcap_data($|[^[0-9a-z$_])/i', $sql, $redcapDataMatches, PREG_OFFSET_CAPTURE)){
