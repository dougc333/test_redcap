...
class  EmailScheduler
{
	...
	public function setMessageQueue($emailSubject, $emailMessage, $useremail_list, $start=0)
	{
		...
		$closure = function() use($username, $senderEmail, $emailSubject, $emailMessage, $useremail_list, $start) {
			...
			$message = $emailHelper->initMessage($emailSubject, $emailMessage);
			$maxTime = $emailHelper->getDateTimeFromInterval(EmailScheduler::MAX_EXECUTION_TIME);
			...
			for ($index=$start; $index < count($useremail_list); $index++) {
				...
				if($too_much) {
					$emailHelper->setMessageQueue($emailSubject, $emailMessage, $useremail_list, $index);
					break;
				}
				$sent = $emailHelper->sendMessage($message, $useremail);
