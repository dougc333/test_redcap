...
class Form
{
			...
			foreach ($these_fields as $this_field)
			{
				...
				if ($Proj->metadata[$this_field]['element_type'] != "checkbox")
				{
					foreach ($these_fields_data[$this_field] as $this_instance=>$value) {
						...
						$html .= "\n  <input type=\"hidden\" name=\"{$this_field}{$this_instance_name}\" value=\"".htmlspecialchars($value, ENT_QUOTES)."\">";
					}
				}
				// Checkbox field
				else
	...
	public static function getActionTags($onlineDesigner=false)
	{
		...
		if (!$onlineDesigner) {
            ...
            $action_tags['@DOWNLOAD-COUNT'] = $lang['design_951'];
		...
		$action_tags['@CALCTEXT'] = $lang['design_837'].RCView::div(['class'=>'mt-2'], $lang['design_1047']).RCView::div(['class'=>'mt-2'], $lang['design_1000']);
	...
	public static function replaceIfActionTag($misc, $project_id, $record, $event_id, $instrument, $instance)
	{
        ...
        $i = 0;
		while (preg_match("/@IF\s*\(/", $misc) && $i < 10000) {
			...
			if ($ifText != "") {
				...
				$misc = preg_replace("/@IF\s*\(\s*" . preg_quote($ifText, '/') . "\s*\)/", $evaldIfText, $misc);
			}
            $i++;
		}
