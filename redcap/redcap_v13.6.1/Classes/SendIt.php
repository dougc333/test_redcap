	...
	public static function renderUploadPage()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST' && !$upload_errors)
		{
			...
			if (count($errors) == 0)
			{
                ...
                if ($fileLocation == 3) {
                    $sql = "select e.record, e.event_id, e.instance, m.form_name from redcap_edocs_metadata d, redcap_metadata m, redcap_data e  
			...
			if ( $row['pwd'] == md5(trim($_POST['pwd'])) )
			{
				...
				if ($pullFromFileSystem)
				{
                    ...
                    elseif ($edoc_storage_option == '5')
                    {
                        ...
                        $contents = file_get_contents('gs://'.$GLOBALS['google_cloud_storage_api_bucket_name'].'/' . $row['doc_name']);
