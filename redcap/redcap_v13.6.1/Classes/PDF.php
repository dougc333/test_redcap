...
class PDF
{
	...
	public static function output($survey_mode = false, $hideAllHiddenAndHiddenSurveyActionTagFields = false)
	{
		...
		if (function_exists('mb_convert_encoding') && is_dir($pathToPdfUtf8Fonts)) {
            ...
            defined("FPDF_FONTPATH") or define("FPDF_FONTPATH",   APP_PATH_WEBTOOLS . "pdf" . DS . "font" . DS);
            defined("_SYSTEM_TTFONTS") or define("_SYSTEM_TTFONTS", APP_PATH_WEBTOOLS . "pdf" . DS . "font" . DS);
			// Set constant
								...
								// foreach ($this_metadata as $row)
							{
								...
								if ($row['element_type'] == "yesno" || $row['element_type'] == "truefalse" || $row['element_type'] == "radio" || $row['element_type'] == "select" || $row['element_type'] == "advcheckbox" || $row['element_type'] == "checkbox" || $row['element_type'] == "sql")
								{
											...
											if ($row['edoc_display_img']) {
                                                ...
                                                if (getFileExt(strtolower($fname)) == 'pdf') {
                                                    if ($GLOBALS['display_inline_pdf_in_pdf'] == '1' && class_exists('Imagick')) {
                                                        $pdf_to_image = true;
                                                        $pdf_to_image_doc_id = $row['edoc_id'];
                                                        $pdf_to_image_fname = $fname;
                                                    } else {
									...
									if (isset($display_inline_image) && $display_inline_image) {
                                        ...
                                        if ($pdf_to_image)
                                        {
                                            // Get the PDF file - copy to TMP
                                            $imgfile_path = Files::copyEdocToTemp($pdf_to_image_doc_id, true);
                                            if ($imgfile_path !== false) {
                                                // Init the Imagick object
                                                try {
                                                    $imagick = new Imagick();
                                                    $imagick->setResolution(200,200); // Resolution set to 200dpi
                                                    $imagick->readImage($imgfile_path);
                                                    $imagick->setImageFormat('png');
                                                    $tmp_fname = uniqid('pngtopdf');
                                                    $imagick->writeImages(APP_PATH_TEMP.$tmp_fname.'.png', false);
                                                    $firstPagePdf = true;
                                                    for ( $pdf_c = 0; $pdf_c < $imagick->count(); $pdf_c++ ) {
                                                        // Get image size
                                                        $img_file_with_path = "";
                                                        if ( $imagick->count() == 1 ) {
                                                            $img_file_with_path = APP_PATH_TEMP.$tmp_fname.'.png';
                                                        }
                                                        else {
                                                            $img_file_with_path = APP_PATH_TEMP.$tmp_fname.'-'.$pdf_c.'.png';
                                                        }
										...
										if (!$pdf_to_image) $imgfile_path = Files::copyEdocToTemp($row['edoc_id'], true);
										if (!$pdf_to_image && $imgfile_path !== false) {
                                            // Double-check the image's file extension to make sure it matches the mime type (if not, could crash or time out)
                                            if (Files::mime_content_type($imgfile_path) != mime_content_type($imgfile_path)) {
                                                // Rename the file with appropriate file extension
                                                $imgfile_path2 = $imgfile_path.".".Files::get_file_extension_by_mime_type(mime_content_type($imgfile_path));
                                                if (@rename($imgfile_path, $imgfile_path2)) {
                                                    $imgfile_path = $imgfile_path2;
                                                } elseif (file_put_contents($imgfile_path2, file_get_contents($imgfile_path))) {
                                                    unlink($imgfile_path);
                                                    $imgfile_path = $imgfile_path2;
                                                }
                                            }
