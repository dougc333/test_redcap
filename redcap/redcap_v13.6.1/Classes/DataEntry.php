...
class DataEntry
{
			...
			***************************************************************/
			...
			foreach ($Proj->forms as $form_name=>$attr)
			{
				...
				foreach ($form_events as $this_event_id=>$eattr)
				{
                    ...
                    $form_enabled = $form_enabled || $formsAccess[$_GET["id"]][$this_event_id][$form_name];
					// Find collapsed state of column
            ...
            switch ($rr_type)
            {
                ...
                case 'file':
                    ...
                    if ($edoc_field_option_enabled) {
                        ...
                        print '<div id="fileupload-container-'.$name.'" '.$can_preview.' data-file-type="'.($signature_field ? "signature" : "file").'" class="fileupload-container '.$missingClass.'"><a target="_blank" class="filedownloadlink" name="'.$name.'" '.$tabindex.' href=\''.$this_file_link.'\' onclick="incrementDownloadCount(\''.implode(",", Form::getDownloadCountTriggerFields($project_id, $name)).'\',this);return appendRespHash(\''.$name.'\');" id="'.$name.'-link"
                                      ...
                                      $response_id=null, $callSaveRecordHook=false, $surveyJustCompleted=false)
    {
        global $double_data_entry, $user_rights, $table_pk, $require_change_reason, $context_msg_update, $multiple_arms,
               ...
               $realtime_webservice_global_enabled, $randomization, $missingDataCodes, $mycap_enabled;
        ...
        {
            Participant::saveParticipant(PROJECT_ID, $fetched);
        }
...
//                        order by rl.record_int desc limit 1";
        ...
        if ((PAGE == "DataEntry/index.php" || PAGE == "ProjectGeneral/keep_alive.php") && isset($_GET['page']) && isset($_GET['id']) && isset($_GET['event_id']))
        {
            ...
            $sql = "select a.session_id, a.user
                    ...
                    and (((u.data_entry like '%[{$_GET['page']},1]%' OR u.data_entry like '%[{$_GET['page']},3]%') AND ur.role_id IS NULL) 
                        ...
                        OR i.super_user = 1)
                    and a.log_view_id = (select b.log_view_id from redcap_log_view b where b.user = a.user and not b.page in ('MultiLanguageController:ajax', 'DataEntry/web_service_cache_item.php') order by b.log_view_id desc limit 1)
	...
	public static function launchDataEntryTrigger()
	{
        ...
        // Perform piping on the full URL
        $full_url = Piping::replaceVariablesInLabel($full_url, $record, $_GET['event_id'], $_GET['instance'], [], false, PROJECT_ID, true, $params['redcap_repeat_instrument']??"", 1, false, false, $params['instrument']??"");
        // Send Post request
