			...
			// State validation 
				...
				try {
					if ($is_azure_ad_endpoint_v1) {
                        ...
                        // Set user info
						$userinfo = array(
							'userPrincipalName' => $oauth2_azure_ad_username_attribute === 'userPrincipalName' ? $azureADemail : $resource_owner->claim('preferred_username'),
							'mail' => $azureADemail,
    ...
    private static function initOpenIDConnect()
    {
        try {
            ...
            if (isset($configParams['scopes_supported']) && is_array($configParams['scopes_supported']) && !empty($configParams['scopes_supported'])) {
                $oidc->addScope($configParams['scopes_supported']);
            } else {
