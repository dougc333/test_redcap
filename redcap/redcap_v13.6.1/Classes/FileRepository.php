...
class FileRepository
{
    ...
    public static function getFileList($folder_id=null, $type=null, $recycle_bin=0)
	{
            ...
            if ($file_repository_enabled)
            {
                ...
                if (!FileRepository::userHasFolderAccess($folder_id) || $noDagAccess) {
                    ...
                    while ($row = db_fetch_row($q)) {
                        ...
                        $filename = $row[1];
                        $dateDeleted = $row[6];
                        $row[1] = "<i class='$faClass fs18 me-1'></i> <a id='file-download-$doc_id' href='javascript:;' class='fs14' title='" . RCView::tt_js('docs_75') . "' onclick=\"fileRepoDownload($doc_id);\">$filename</a>"
        ...
        elseif ($type == 'export')
        {
            ...
            while ($row = db_fetch_row($q))
            {
                ...
                if (isset($cannotExportFile[$doc_id])) {
                    $row[1] = "<i class='$faClass fs18 me-1 text-primaryrc'></i> <span class='fs14 text-dangerrc' style='cursor:not-allowed;' title='".RCView::tt_js('data_export_tool_294')."'>$filename</span>";
                } else {
                    $row[1] = "<i class='$faClass fs18 me-1 text-primaryrc'></i> <a href='javascript:;' class='fs14' title='".RCView::tt_js('docs_75')."' onclick=\"fileRepoDownload($doc_id);\">$filename</a>";
                }
                unset($row[0]);
                // Size
                $row[2] = ['sort'=>$row[2], 'display'=>RCView::span(['class'=>'nowrap'], ($row[2] < 1048576 ? round($row[2] / 1024, ($row[2] < 100 ? 3 : 1)) . " KB" : round($row[2] / 1048576, 1) . " MB"))];
        ...
        elseif ($type == 'record_lock_pdf_archive')
        {
            ...
            foreach ($files as $file)
            {
                ...
                if ($user_rights['data_export_tool'] == '1') {
                    $filename = "<div class='nowrap'><i class='fa-solid fa-file-pdf fs18 me-1 text-dangerrc'></i> <a href='javascript:;' class='fs14' title='".RCView::tt_js('docs_75')."' onclick=\"fileRepoDownload($doc_id,'lock_doc_id');\">$filename</a></div>";
                } else {
        ...
        elseif ($type == 'pdf_archive')
        {
            ...
            foreach ($files as $file)
            {
                ...
                $filename = $file['doc_name'];
                if ($user_rights['data_export_tool'] == '1') {
                    $filename = "<div class='nowrap'><i class='fa-solid fa-file-pdf fs18 me-1 text-dangerrc'></i> <a href='javascript:;' class='fs14' title='".RCView::tt_js('docs_75')."' onclick=\"fileRepoDownload($doc_id,'doc_id');\">{$file['doc_name']}</a></div>";
                } else {
                    $filename = "<div class='nowrap'><i class='fa-solid fa-file-pdf fs18 me-1 text-dangerrc'></i> $filename</div>";
                }
                $row[] = "";
    ...
    public static function renderIndexPage()
    {
        ...
        addLangToJS(['period', 'docs_74', 'sendit_32', 'docs_77', 'docs_78', 'docs_79', 'docs_80', 'docs_81', 'docs_82', 'sendit_03', 'sendit_04', 'sendit_05', 'global_01',
            ...
            'docs_1145', 'docs_1146', 'docs_1147', 'global_03', 'docs_1149', 'docs_1150']);
