                ...
                foreach ($forms as $form)
                {
                    ...
                    if (isset($events_forms[$event_id][$form])) {
                        ...
                        foreach ($events_forms[$event_id][$form] as $cond) {
                            $passedLogicTest = REDCap::evaluateLogic($cond, $Proj->project_id, $id, $event_id, 1, ($Proj->isRepeatingFormOrEvent($event_id, $form) ? $form : ""), $current_instrument, self::$cached_data);
                            if ($passedLogicTest) {
                                $access = true;
                                break;
                            }
