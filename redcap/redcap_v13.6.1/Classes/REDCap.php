                ...
                if (!$recordExists) {
                    ...
                    if ($Proj->longitudinal && $Proj->multiple_arms) {
                        if (!empty($arm)) {
                            ...
                            $recordArms = Records::getArmsForAllRecords($project_id);
...
$deleted = REDCap::deleteRecord(851, $record, null, $event, null, $instance);
     ...
     */
    public static function deleteRecord($project_id, $record, $arm = null, $event = null, $instrument = null, $repeating_instance = 1)
    {
        global $lang;
        $appendLoggingDescription = '';
        if (defined("PAGE") && strtolower(PAGE) == 'api/index.php') {
                    ...
                    if (!empty($event)) { // Return Entire event's data
                        ...
                        foreach ($armIds as $armId) {
                            ...
                            $log_event_id = Records::deleteRecordByProject($project_id,
                                ...
                                $armId,
                                $appendLoggingDescription,
								$Proj->project['allow_delete_record_from_log']);
                        }
						if (isset($event_id_orig)) {
							$_GET['event_id'] = $event_id_orig;
						} else {
	...
	public static function setFormName($project_id, $form_name, $new_form_name)
	{
		...
		if ($status < 1)
		{
			...
			$sql = "update redcap_user_rights set
                    data_entry = replace(data_entry, '[{$form_name},', '[$new_form_name,'),
                    data_export_instruments = replace(data_export_instruments, '[{$form_name},', '[$new_form_name,')
     ...
     * DESCRIPTION: int <b>REDCap::storeFile</b> ( string <b>$filepath</b> [, int <b>$project_id</b>] )
...
$doc_id = REDCap::storeFile($filepath, $project_id);
     ...
     */
    public static function storeFile($filepath, $project_id = null)
    {
