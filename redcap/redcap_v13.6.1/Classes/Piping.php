...
//                'survey-last-instance-sentorscheduled:'.$lang['global_277'] => array($lang['piping_79'], array("<code>[survey-duration-completed:prescreener]</code>", "93"), array("<code>[survey-duration-completed:prescreener:m]</code>", "12.7"), array("<code>[visit_1_arm_1][survey-duration-completed:prescreener:d][last-instance]</code>", "3.89"))
			...
			$lang['global_181']=>array(
				...
				'scatter-plot:'.$lang['global_284'].','.$lang['global_285'].','.$lang['global_286'].':'.$lang['global_280'] => array($lang['global_195'], array("<code>[scatter-plot:height]</code>", $lang['global_190']), array("<code>[scatter-plot:height,weight]</code>", $lang['global_190']), array("<code>[scatter-plot:height,weight,gender]</code>", $lang['global_190'])),
				'line-chart:'.$lang['global_284'].','.$lang['global_285'].','.$lang['global_286'].':'.$lang['global_280'] => array($lang['global_196'], array("<code>[line-chart:visit_date,weight]</code>", $lang['global_190']), array("<code>[line-chart:visit_date,weight,gender]</code>", $lang['global_190'])),
				'bar-chart:'.$lang['global_282'].','.$lang['global_286'].':'.$lang['global_280'] => array($lang['global_194'], array("<code>[bar-chart:race]</code>", $lang['global_190']), array("<code>[bar-chart:race,gender]</code>", $lang['global_190'])),
...
//                        }
                    ...
                    case "mycap-participant-code" :
                        $wrapThisItem = true;
                        $participant_code = Participant::getRecordParticipantCode($project_id, $record);
...
//		 print_array($repeating_instruments);
		...
		foreach ($fields as $key=>$this_field)
		{
			...
			if ($has_data_value)
			{
				...
				elseif (!$returnDatesAsYMD && $field_validation !== null && substr($field_validation, 0, 4) == 'date' && (substr($field_validation, -4) == '_mdy' || substr($field_validation, -4) == '_dmy') && !in_array($this_field, $deidFieldsRemoved)) {
				...
				} elseif ($field_type == 'text' && in_array('ampm', $mc_field_params[$key]) && in_array($field_validation, ['time', 'datetime', 'datetime_ymd', 'datetime_mdy', 'datetime_dmy', 'datetime_seconds', 'datetime_seconds_ymd', 'datetime_seconds_mdy', 'datetime_seconds_dmy'])) {
					$this_mc_param = '-ampm';
					// MC fields
				} else {
					$replaceWithValue = in_array('value', $mc_field_params[$key]);
			...
			if ($mlm_active && in_array($field_type, ["slider","radio","yesno","truefalse","select","checkbox"], true)) {
				$span_attrs["data-piperec-instance"] = $this_instance == "" ? "1" : $this_instance;
