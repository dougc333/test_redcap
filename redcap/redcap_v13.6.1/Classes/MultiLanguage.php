 ...
 * 
 * - SPECIAL ATTENTION:
 *   - Radio HTML when using RTL languages (like Hebrew or Arabic)
 *   - PDF considerations: see https://community.projectredcap.org/post.php?id=91212&comment=113698
 * 
 * - Stuff in REDCap that should be addressed:
 *  - Randomization dialog (randomizeDialog(), Project.js) has HARDCODED strings!
 *    Best approach may be to provide JS strings in Data Entry
 ...
 *   - Set languages and translations for Survey::displayAccessCodeForm()
 *   - Or survey_550 and survey_551
 * 
 * - OTHER TODOs:
...
class MultiLanguage
{
     ...
     * @return Array 
     */
    public static function getSystemLanguage($data) {
        // Default response
        $lang_id = json_decode($data);
        $settings = self::getSystemSettings();
        $mlm_lang = $settings["langs"][$lang_id];
        if (!is_array($mlm_lang)) {
            return self::response(false, RCView::tt_i_strip_tags("multilang_102", array($lang_id))); // The language '{0}' is not available.
        }
        ...
        foreach ($ui_meta as $key => $meta) {
            if ($meta["type"] == "bool") {
                ...
                $response["lang"]["uiOptions"][] = array(
                    "id" => $key,
                    "value" => (isset($mlm_lang["ui"][$key]) && $mlm_lang["ui"][$key] == true)
                ); 
            }
            else {
                // Only copy translations if they are set
                if (isset($mlm_lang["ui"][$key]) && !self::isEmpty($mlm_lang["ui"][$key])) {
                    $response["lang"]["uiTranslations"][] = array(
                        "id" => $key,
                        "translation" => $mlm_lang["ui"][$key]["translation"],
                        "hash" => $mlm_lang["ui"][$key]["refHash"],
                    );
                }
            }
        }
    ...
    public static function getTranslationSettings(Context $context, $do_piping, $blocklist = []) {
        ...
        foreach($active_langs as $lang_id) {
                ...
                foreach (["field-enum"] as $type) {
                    ...
                    if (in_array($field_meta["type"], ["select","sql"]) && count($ref["mdcs"])) {
                        foreach ($ref["mdcs"] as $mdc => $_) {
                            $field[$type][$mdc] = $this_orig["dd"]["mdc-label"][$mdc][""]["translation"] ?? "";
                        }
                    }
                }
                ...
                if ($mg_name) {
                    ...
                    if (isset($this_orig["dd"]["matrix-enum"][$mg_name])) {
                        foreach ($this_orig["dd"]["matrix-enum"][$mg_name] as $index => $value) {
                            $this_lang["field-translations"][$field_name]["field-enum"][$index] = $do_piping ? self::performPiping($field_context, $value["translation"], true, $lang_id) : $value["translation"];
                        }
                    }
                    if (isset($this_orig["dd"]["matrix-header"][$mg_name])) {
                        $this_lang["field-translations"][$field_name]["field-header"] = $do_piping ? self::performPiping($field_context, $this_orig["dd"]["matrix-header"][$mg_name][""]["translation"], true, $lang_id) : $this_orig["dd"]["matrix-header"][$mg_name][""]["translation"];
                    }
                }
            }
            ...
            if ($context->is_dataentry) {
                ...
                foreach ($ref["forms"] as $form_name => $_) {
                    $this_lang["form-name"][$form_name] = $this_orig["dd"]["form-name"][$form_name][""]["translation"] ?? "";
                }
                ...
                foreach ($ref["events"] as $event_id => $_) {
                    $this_lang["event-name"][$event_id] = $this_orig["dd"]["event-name"][$event_id][""]["translation"] ?? "";
                    $this_lang["event-custom_event_label"][$event_id] = $this_orig["dd"]["event-custom_event_label"][$event_id][""]["translation"] ?? "";
                }
                foreach ($ref["mdcs"] as $mdc => $_) {
                    $this_lang["mdc-label"][$mdc] = $this_orig["dd"]["mdc-label"][$mdc][""]["translation"] ?? "";
                }
            }
            ...
            if ($context->is_survey) {
                // Survey login error
                $this_lang["sq-translations"] = array(
                    "sq-survey_auth_custom_message" => $this_orig["dd"]["sq-survey_auth_custom_message"][""][""]["translation"] ?? ""
                );
            }
            ...
            if (isset($ref["surveys"][$context->instrument])) {
                foreach ($ref["surveys"][$context->instrument] as $survey_element => $ref_data) {
                    ...
                    if (starts_with($survey_element, "survey-")) {
                        $this_lang["survey-translations"][$survey_element] = replaceUrlImgViewEndpoint(isset($this_orig["dd"][$survey_element][$context->instrument][""]["translation"]) ? ($do_piping ? self::performPiping($context, $this_orig["dd"][$survey_element][$context->instrument][""]["translation"], false, $lang_id) : $this_orig["dd"][$survey_element][$context->instrument][""]["translation"]) : "");
                        $translation_settings["ref"]["survey-translations"][$survey_element] = replaceUrlImgViewEndpoint($do_piping ? self::performPiping($context, $ref_data["reference"], false, $lang_id) : $ref_data["reference"]);
                    }
                }
            }
            // User interface translations (use language string id as key where possible)
        ...
        foreach ($fields as $field_name) {
            ...
            foreach(["field-label", "field-note", "field-header"] as $type) {
                ...
                $field[$type] = replaceUrlImgViewEndpoint($reference);
            }
            foreach(["field-video_url"] as $type) {
                $value = $field[$type]["reference"] ?? "";
                if ($value != "") {
            ...
            foreach(["field-enum"] as $type) {
                ...
                if (is_array($to_process)) {
                    foreach ($to_process as $index => $value) {
                        ...
                        $reference = $do_piping ? self::performPiping($field_context, $value, true, $ref_lang) : $value;
                        $field[$type][$index] = $reference;
                    }
                }
                // Add missing data codes to select type fields
                if (in_array($field["type"], ["select","sql"]) && count($ref["mdcs"])) {
                    foreach ($ref["mdcs"] as $mdc => $_) {
                        $field[$type][$mdc] = $ref["mdcs"][$mdc]["reference"] ?? "";
                    }
                }
                ...
                foreach ($mg_ref as $type => $td) {
                    if ($type == "matrix-enum") {
                        foreach ($td as $index => $value) {
                            $mg_ref[$type][$index] = $do_piping ? self::performPiping($field_context, $value["reference"], true, $ref_lang) : $value["reference"];
                        }
                    }
                    else if (starts_with($type, "matrix-")) {
                        $mg_ref[$type] = $do_piping ? self::performPiping($field_context, $td["reference"], true, $ref_lang) : $td["reference"];
                    }
                }
        ...
        if ($context->is_dataentry) {
            ...
            foreach ($ref["forms"] as $form_name => $form_data) {
                $translation_settings["ref"]["form-name"][$form_name] = $form_data["form-name"]["reference"] ?? "";
            }
            // Event names and custom event labels
            foreach ($ref["events"] as $event_id => $event_data) {
                $translation_settings["ref"]["event-name"][$event_id] = $event_data["event-name"]["reference"] ?? "";
                $translation_settings["ref"]["event-custom_event_label"][$event_id] = $event_data["event-custom_event_label"]["reference"] ?? "";
            }
            // Missing data code labels
            foreach ($ref["mdcs"] as $mdc => $mdc_data) {
                $translation_settings["ref"]["mdc-label"][$mdc] = $mdc_data["reference"];
            }
        }
        // Items needed for surveys only
        if ($context->is_survey) {
            // Login error message
            $translation_settings["ref"]["sq-translations"]["sq-survey_auth_custom_message"] = $ref["surveyQueue"]["sq-survey_auth_custom_message"]["reference"] ?? "";
        }
        // Finally, add UI reference
        foreach ($ui_meta as $ui_key => $ui_data) {
    ...
    public static function getProjectSettings($project_id, $tryDraftMode=false)
	{
        ...
        foreach ($meta["asis"] as $_ => $asi) {
            if (!isset($settings["asiSources"][$asi["form"]])) {
                $settings["asiSources"][$asi["form"]] = "user";
            }
        }
    ...
    public static function save($project_id, $data) {
        try {
            ...
            // Only admins can set debug or enable on projects
            if (!UserRights::isSuperUserNotImpersonator()) {
                unset($global_settings["debug"]);
                unset($global_settings["admin-enabled"]);
                unset($global_settings["admin-disabled"]);
            }
            ...
            $store_config = function($lang_id, $name, $new, $old, $type) use ($project_id, $pmd, $pmd_map, &$log_active_changed, &$log_visible_changed, &$log_num_items_updated) {
                if ($type == "bool") {
                    $new = $new ? "1" : "";
                }
                else if ($type == "list") {
            ...
            foreach ($global_settings as $name => $meta) {
                ...
                $store_config(null, $name, $new_value, $old_value, $meta["type"]);
            }
            foreach ($langs_in_data as $lang_id) {
            ...
            foreach ($data["langs"] as $lang_id => $this_lang) {
                ...
                foreach (self::getUIMetadata($is_system) as $item => $meta) {
            ...
            // Cleanup - Delete all data for non-existing languages from all tables
    ...
    public static function getUIMetadata($is_system = false) {
        ...
        $metadata = array (
            ...
            "global_120" => [
                ...
                "type" => "string",
                "id" => "global_120",
                "default" => $lang["global_120"], // Watch video
                "prompt" => RCView::tt_i("multilang_335", [$lang["global_120"]]), //= The '{0}' button label:
                ],
            "data_entry_502" => [
                "category" => "ui-fields",
                "group" => "field_desc",
                "type" => "string",
                "id" => "data_entry_502",
                "default" => $lang["data_entry_502"], // Video
                "prompt" => RCView::tt_i("multilang_329", [$lang["data_entry_502"]]), //= The '{0}' popup window title:
            ],
            "design_204" => [
                "category" => "ui-fields",
                "group" => "field_desc",
    ...
    private static function getProjectGeneralSettings($project_id, $timestamp) {
        ...
        foreach ($pmd["forms"] as $form => $form_data) {
            // dataEntryEnabled
            foreach ($settings["langs"] as $lang_id => $this_lang) {
                $proj_export["dataEntryEnabled"][$form][$lang_id] = isset($this_lang["dd"]["survey-active"][$form]);
            }
            if ($form_data["isSurvey"]) {
                // surveyEnabled
                foreach ($settings["langs"] as $lang_id => $this_lang) {
                    $proj_export["surveyEnabled"][$form][$lang_id] = isset($this_lang["dd"]["form-active"][$form]);
                }
                // excludedSettings
        ...
        foreach ($pmd["asis"] as $_=> $asi_data) {
            // asiSources
            $proj_export["asiSources"][$asi_data["form"]] = $settings["asiSources"][$asi_data["form"]] ?? "user";
        }
        ...
        while ($row = db_fetch_assoc($result)) {
            ...
            foreach ($lang_kv_pairs as $lang_kv) {
                if (!empty($lang_kv)) {
                    ...
                    $langs[$kv[0]] = htmlentities($kv[1], ENT_QUOTES);
                }
            }
            $n_langs = count($langs);
            ...
            $stat = [
                ...
                "projectLanguage" => $row["project_language"] == "English" ? "" : htmlentities($row["project_language"], ENT_QUOTES),
                "nAllLangs" => $n_langs,
    ...
    public static function getChoiceLabelTranslation(Context $context, $field_name, $code) {
        ...
        $settings = self::getProjectSettings($context->project_id);
        $is_ref_lang = $lang_id == $settings["refLang"];
        $translation = $settings["langs"][$lang_id]["dd"]["field-enum"][$field_name][$code]["translation"] ?? "";
        ...
        if (self::isEmpty($translation)) {
            $ref = self::getProjectMetadata($context->project_id);
            $translation = $ref["fields"][$field_name]["field-enum"][$code]["reference"] ?? "";
        }
        return $translation;
    }
    ...
    public static function getCurrentLanguage(Context $context = null, $do_not_use_ultimate_fallbacks = false) {
        ...
        $lang_id = isset($context->is_dataentry) && $context->is_dataentry ? "" : ($_COOKIE[self::SURVEY_COOKIE] ?? "");
    ...
    /**
     * Handles the keep-alive ajax request (system setup page)
     */
    ...
    private static $global_settings_project = array(
        ...
        "admin-disabled" => [ "type" => "bool", "default" => false ],
        "refLang" => [ "type" => "string", "default" => ""] ,
        "fallbackLang" => [ "type" => "string", "default" => ""] ,
        "disabled" => [ "type" => "bool", "default" => false ],
        "highlightMissingDataentry" => [ "type" => "bool", "default" => false ],
        "highlightMissingSurvey" => [ "type" => "bool", "default" => false ],
        "autoDetectBrowserLang" => [ "type" => "bool", "default" => false ],
        "alertSources" => [ "type" => "json", "default" => "" ],
        "asiSources" => [ "type" => "json", "default" => "" ],
        "excludedAlerts" => [ "type" => "list", "default" => "" ],
        "excludedFields" => [ "type" => "list", "default" => "" ],
        "excludedSettings" => [ "type" => "json", "default" => "" ],
        "designatedField" => [ "type" => "string", "default" => "" ],
    );
     ...
     * @param string $value The language id to validate
     * @return string The validated language id
     * @throws Exception in case the validation fails
     */
    private function validateLangKey($value) {
        $key = trim($value);
        $re = "/^[a-zA-Z](?:[a-zA-Z-]*[a-zA-Z])?$/m";
        if (preg_match($re, $key)) {
            return $key;
        }
        throw new Exception("MultiLanguage Management: Invalid language ID.");
    }
