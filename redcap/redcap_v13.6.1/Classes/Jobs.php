...
class Jobs
{
    ...
    public function ClinicalDataMartDataFetch()
    {
		...
		foreach ($revisions as $revision) {
			try {
				$bgRunner->schedule($revision->id, $mrn_list=[], $sendFeedback=true);
				$scheduled_projects++;
			} catch (\Exception $e) {
	...
	public function RemoveOutdatedRecordCounts()
	{
		...
		db_query($sql);
		// Remove old record counts for projects that have had some logged activity in the past 2 weeks
		// if the time of the last record count or the time of last cache has been more than 7 days.
		$daysOldCounted = 7;
		$daysOldEvent = 14;
	...
	{
        // Remove all errors from the error log table older than 30 days (to free up space)
