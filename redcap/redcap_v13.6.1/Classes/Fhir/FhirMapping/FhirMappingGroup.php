...
class FhirMappingGroup
{
   ...
   * @param array $mapping_data metadata associated with the mapping base on metadata_source
   * @return void
   */
   ...
   */
  private function getBestDate($date_a, $date_b, $strategy) {
   ...
   * @return void
   */
  private function parseMapping(FhirMapping $mapping) {
   ...
   * @return void
   */
  private function parseDates(FhirMapping $mapping, $metadata_array=[]) {
    $temporal = $metadata_array[$mapping->getName()]['temporal'] ?? false;
   ...
   * @return void
   */
  private function addUniqueProperty($name, $value) {
   ...
   * @return void
   */
  private function parseField($mapping) {
   ...
   * @return void
   */
  private function parseOptions($mapping) {
    $options = $mapping->getOptions();
    foreach ($options as $option) {
      $this->addUniqueProperty('options', $option);
    }
  }
   ...
   * @return void
   */
  private function parseSubcategories($mapping, $mapping_data=[]) {
