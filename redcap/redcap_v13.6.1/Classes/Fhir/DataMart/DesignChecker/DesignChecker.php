...
class DesignChecker
{
    ...
    public function projectIsDraftMode() {
        ...
        return intval(@$this->project->project['draft_mode'])===1;
    }
    ...
    {
        if($temp) return @$this->project->forms_temp;
    ...
    public function check() {
        ...
        foreach ($blueprintForms as $formIndex => $form) {
            ...
            if(!$formExists) {
                ...
                $this->addCommand(new CreateFormCommand($projectDesigner, $formName, $formLabel));
                // check all variables
    ...
    public function executeCommands() {
        ...
        try {
            ...
            $response['affected_rows'] = db_affected_rows();
    ...
    public function restoreCommands() {
        $cache = $this->getCache();
        $commands = @unserialize($cache->get('commands')) ?: [];
    ...
    public function sectionHeaderExists($formName, $text)
    {
        $sectionHeaderExists = function($temp=false) use($formName, $text) {
            ...
            $found = array_filter($metadata, function($field) use($formName, $text) {
                if(@$field['form_name'] != $formName) return false;
                return @$field['element_preceding_header'] == $text;
            });
            return !empty($found);
        };
    ...
    public function sectionHeaderBelongsToAnotherVariable($formName, $variableName, $text)
    {
        $sectionHeaderBelongsToAnotherVariable = function($temp=false) use($formName, $variableName, $text) {
            ...
            foreach ($metadata as $key => $field) {
                if(@$field['form_name'] != $formName) continue;
                if(@$field['element_preceding_header'] != $text) continue;
    ...
    public function getVariableMetadataFromProject($variableName, $temp=false)
    {
        ...
        foreach ($projectToBlueprintMapping as $projectKey => $blueprintKey) {
            $metadata[$blueprintKey] = @$metadata[$projectKey];
        }
    ...
    {
        $getVariableSettingsMismatch = function($temp=false) use($variableName, $settings) {
            ...
            foreach ($settings as $key => $blueprintSetting) {
                ...
                $variableSetting = @$variable[$key];
                if($variableSetting!=$blueprintSetting)
                $notMatching[$key] = $blueprintSetting;
            }
            return $notMatching;
        };
        if($this->projectIsDraftMode()) return $mismatchAsDraft = $getVariableSettingsMismatch(true);
        return $mismatch = $getVariableSettingsMismatch();      
    }

    public function getProjectPrimaryKey($temp=false) {
        if(!$temp) return @$this->project->table_pk;
        else return @$this->project->table_pk_temp;
    }
