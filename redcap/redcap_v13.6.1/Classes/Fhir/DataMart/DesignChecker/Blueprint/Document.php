    ...
    function getFormVariables($formName)
    {
        ...
        $choicesToString = function($choices) {
            ...
            foreach ($choices as $choice) {
                $strings[] = $choiceToString = sprintf('%s, %s', @$choice['value'], @$choice['label']);
            }
        ...
        $getRangeValue = function($itemDef, $comparator) use($xpath) {
            ...
            $value = $xpath->query(sprintf('.//x:RangeCheck[@Comparator="%s"]/CheckValue', $comparator), $itemDef)->item(0)->nodeValue ?: '';
            return $value;
        };
