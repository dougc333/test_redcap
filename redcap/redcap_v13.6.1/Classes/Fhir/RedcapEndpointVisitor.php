...
use Vanderbilt\REDCap\Classes\Fhir\Endpoints\R4\DocumentReference;
...
class RedcapEndpointVisitor implements EndpointVisitorInterface
{
  ...
  {
    $options = $this->mappingGroup->getOptions();
    ...
    switch ($class) {
      ...
      case MedicationRequest::class:
        $options = $this->visitMedications($endpoint, $options);
        break;
   ...
   * @return array
   */
  public function visitMedications($endpoint, $options)
  {
  ...
  protected function makeDateRange($date_min, $date_max)
  {
     ...
     * @see https://www.hl7.org/fhir/search.html#date
     */
    $convertFromGmt = function(&$date_min, &$date_max, $timezoneID) use($applyTimeZone){
      ...
      if($date_min instanceof \DateTime) $date_min = $applyTimeZone($date_min, $timezoneID);
      if($date_max instanceof \DateTime) $date_max = $applyTimeZone($date_max, $timezoneID);
    };
    ...
    $formatDate = function($datetime, $timezoneID) use($getFormattedTimezone) {
      ...
      if($fhirCode===FhirVersionManager::FHIR_DSTU2) {
        // for DSTU return just the date since time does not work (in Epic)
    ...
    $convertFromGmt($date_min, $date_max, $timezoneID);
