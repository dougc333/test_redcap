...
class ClynicalDataPull {
    public function getRecords($project_id, $event_id) {
		$queryString = "SELECT DISTINCT record FROM redcap_data WHERE project_id=? AND event_id=?";
	...
	public function getFhirParams($project_id, $record, $fhirField, $mapping, $day_offset=null, $day_offset_plusminus="+-") {
		...
		if(!@$info['temporal_field']) return $params;
		
		$queryString = "SELECT DISTINCT value FROM redcap_data WHERE project_id=? AND event_id=? AND record=? AND field_name=?";
		$result = db_query($queryString, [$project_id, $eventID, $record, @$info['temporal_field']]);
