...
class CacheEntry
{
    ...
    */
    ...
    {
        $this->project_id = intval(@$db_row['project_id']);
        $this->md_id = intval(@$db_row['md_id']);
        $this->event_id = intval(@$db_row['event_id']);
        ...
        $this->offset_seconds = (@$db_row['offset_seconds'] !== null) ? intval(@$db_row['offset_seconds']) : false;
        $this->adjudicated = boolval(@$db_row['adjudicated']);
        $this->exclude = boolval(@$db_row['exclude']);
     ...
     * this will bve used as subquery (e.g. apply project ID and record filters)
     *
     * @return string
     */
    public static function getCacheEntriesQuery() {
        $queryString = sprintf(
            "SELECT cache.*,
            ...
            LEFT JOIN redcap_data AS data ON (data.project_id=records.project_id AND data.record=records.record AND data.event_id=mapping.event_id AND data.field_name=mapping.field_name)
            LEFT JOIN redcap_data AS temporal_data ON (temporal_data.project_id=records.project_id AND temporal_data.record=records.record AND temporal_data.event_id=mapping.event_id AND temporal_data.field_name=mapping.temporal_field)
