...
class AutoAdjudicator
{
    ...
    */
    ...
    {
        $entriesSubQuery = CacheEntry::getCacheEntriesQuery();
        $queryString = sprintf(
            "SELECT * FROM ($entriesSubQuery) AS sub
            WHERE sub.project_id=%u AND sub.record=%s",
    ...
    public function processRecords($background, $send_feedback) {
        ...
        while($field = $fieldsGenerator->current()) {
            ...
            $total++;
            list('adjudicated' => $adjudicatedCount, 'excluded' => $excludedCount, 'error' => $error) = $this->processField(@$field['record'], @$field['event_id'], @$field['field_name']);
            // mark as adjudicated and excluded if no errors
    ...
    public function getDdpRecordsMetadata($offset=0)
    {
        $loadData = function($offset) {
            ...
            $totalResult = db_query($query_string);
            ...
            $query_string .= sprintf(" LIMIT %u, %u", $offset, self::RECORDS_METADATA_CHUNK_SIZE);
            $result = db_query($query_string);
            if(!$result) throw new \Exception(sprintf("There was an error retrieving cached data from the database for project %u", $this->project_id), 400);
            $data = [];
