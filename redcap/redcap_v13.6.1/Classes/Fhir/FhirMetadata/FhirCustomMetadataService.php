...
class FhirCustomMetadataService
{
  ...
  */
  const MAPPING_HEADERS = ['field','label','description','temporal','category','subcategory','identifier'];
  ...
  * @throws Exception
  */
  public function uploadCustomMapping($fileData) {
    $fileContent = file_get_contents($fileData["tmp_name"]);
    
    $validHeaders = CSVHelper::checkHeadersFromString($fileContent, self::MAPPING_HEADERS);
    if(!$validHeaders) throw new Exception("Invalid format: please use the template as reference", 400);
    $hasData = CSVHelper::hasData($fileContent, 2); // check if there is at least 1 line of data beside the headers
    if(!$hasData) throw new Exception("Invalid format: the file must contain at least 1 mapping", 400);
  ...
  public function validateCustomMappingFile($file) {
    $header = ['field','label','description','temporal','category','subcategory','identifier'];
    $validCategories = ['Laboratory', 'Vital Signs'];
    ...
    $validationStrategies =[
        ...
        new FhirCustomMetadataCategoryValidation($validCategories),
    ];
