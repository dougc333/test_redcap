...
class FhirClient
{
  ...
  public function getPatientID($mrn)
  {
    ...
    $searchPatientOnDatabase = function($mrn) {
      $query_string = "SELECT patient FROM redcap_ehr_access_tokens WHERE mrn=?";
      $result = db_query($query_string, [$mrn]);
   ...
   * @return void
   */
  public function fetchData($mrn, $fhirMetadataSource, $mapping_list=[])
  {
    ...
    $mappingGroups = $this->groupMapping($fhirMetadataSource, $mapping_list);

    foreach ($mappingGroups as $category => $mappingGroup) {
      ...
      unset($mappingGroups[$category]); // free some memory
    }
    // $this->mrn = null; // reset the reference to the current $mrn

  }

  public function fetchDataUsingMappingGroup($patientID, $category, $mappingGroup, &$groupedData=[])
  {
    $response = $this->getEntries($patientID, $category, $mappingGroup);
    $response->fhir_id = $patientID;
    $groupedData[$category] = $response->entries;
  ...
  public function sendRequest($request)
  {
    ...
    try {
      ...
      $clientResponse = new FhirClientResponse($request, $this->project_id);
  ...
  */
   ...
   * @return array
   */
  private function groupMapping($fhirMetadataSource, $fhirMappings)
  {
    ...
    foreach ($fhirMappings as $fhirMapping) {
      ...
      if($mappingData['disabled']) continue; //skip disabled mappings (e.g. adverse events or emails)

      $category = $mappingData['category'];
      if(!$category) continue;
      if(!array_key_exists($category, $groups)) {
        $groups[$category] = new FhirMappingGroup($category);
      }
