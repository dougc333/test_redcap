...
class SurveyScheduler
{
		...
		foreach ($availableSchedules as $survey_id=>$events)
		{
			foreach (array_keys($events) as $event_id)
            {
                ...
                if ($readyToSchedule) {
                    ...
                    // Schedule the participant's survey invitation to be sent by adding it to the scheduler_queue table
                    $invitationWasScheduled = $this->scheduleParticipantInvitation($survey_id, $event_id, $record, 1, false, $addRecurrenceSchedule);
                    if ($invitationWasScheduled) {
                        // Increment number of invitations scheduled just now
	...
	public function scheduleParticipantInvitation($survey_id, $event_id, $record, $instanceNum=1, $isRecurring=false, $addRecurrenceSchedule=false)
	{
	...
	public static function emailInvitations($ssq_id=null)
	{
        ...
        ## SEND INVITATIONS FOR RECURRING ASI'S
            ...
            while ($row = db_fetch_assoc($q))
            {
                ...
                if (Survey::isResponseCompleted($row['survey_id'], $row['record'], $row['event_id'], $instanceNum)) {
                    ...
                    if ($conditionsPassedLogic) {
                        $wasSent = $ss_obj->scheduleParticipantInvitation($row['survey_id'], $row['event_id'], $row['record'], $instanceNum, true);
                    }
	...
	public function displayAutoInviteSurveyEventCheckboxList($project_id)
	{
		...
		return RCView::p(array('class'=>'mt-0'), $lang['asi_025']) .
						...
						RCView::div(array('class'=>'gray mb-0'),
							...
							RCView::a(array('class'=>'ms-5 fs11', 'onclick'=>"$('#reeval_asi_dlg input[type=checkbox]').prop('checked',true);", 'style'=>'text-decoration:underline'), $lang['survey_41']) .
							RCView::a(array('class'=>'ms-2 fs11', 'onclick'=>"$('#reeval_asi_dlg input[type=checkbox]').prop('checked',false);", 'style'=>'text-decoration:underline'), $lang['survey_42']) .
							$html
						);
	}
