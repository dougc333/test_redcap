...
class ExternalModules
{
	...
	{
		/**
		 * The following is used for temporary troubleshooting.
		 */
		// if($moduleDirectoryPrefix === 'flightTracker' && in_array($key, ['ft_data', 'emails_last_run'])){
		// 	$timestamp = (new DateTime)->format("m-d-Y H:i:s.u");
		// 	error_log("$timestamp - ExternalModules::setSetting('$moduleDirectoryPrefix', '$projectId, '$key', \$value, '$type') - Value length is " . strlen(json_encode($value)));

		// 	$tracePath = sys_get_temp_dir() . '/external-modules-set-setting-trace';
		// 	if(!file_exists($tracePath)){
		// 		file_put_contents($tracePath, json_encode(debug_backtrace(), JSON_PRETTY_PRINT));
		// 	}
		// }
	...
	static function &getTestSettings($prefix, $projectId)
	{
	...
	static function filterHookResults($results, $hookName) {
		...
		if(in_array($hookName, ['pdf', 'custom_verify_username'])){
     ...
     */
    public static function getREDCapProjectObject($pid): ?\Project{
        ...
        if(empty($project->project)){
            if(static::isTesting()){
                throw new \Exception('Project found but not found: ' . $pid);
            }
	...
	public static function renderDocumentation(){
		...
		$path = static::getSafePath($page, __DIR__ . '/../docs');
		...
		<div id='ext_mods_docs_container'>
			<style>
				...
				pre{
					/**
					 * Fix display issues on some screen sizes.
					 */
	...
	static function getPrefix(){
		 ...
		 */
		return htmlentities($_GET['prefix'] ?? '', ENT_QUOTES);
	}
	...
	static function getArmById($project, $armId){
		foreach($project->events as $armNumber=>$arm){
