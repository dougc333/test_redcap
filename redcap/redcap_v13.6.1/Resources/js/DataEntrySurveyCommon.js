$(function(){
    ...
    initTinyMCEglobal('notesbox-richtext-left');
    initTinyMCEglobal('notesbox-richtext-right', true);
...
function openEmbedVideoDlg(video_url,unknown_video_service,field_name) { 
    ...
    $('#'+dlgid)
        ...
        .dialog({ height: (vidheight+130), width: (isMobileDevice ? $(window).width() : (vidwidth+60)), open:function(){ fitDialog(this); }, close:function(){ $(this).dialog('destroy'); $('#'+dlgid).remove(); },
            buttons: [{ text: window.lang.calendar_popup_01, click: function(){ $(this).dialog('close'); } }], title: window.lang.data_entry_502, bgiframe: true, modal: true
        });
...
function filePopUp(field_name, signature, replace_version) {
    ...
    var labelText = $('#label-'+field_name+' div[data-kind=field-label]').text()
...
function autoFillRow(tr)
{
    var date_types = new Array('date_ymd', 'date_mdy', 'date_dmy', 'datetime_ymd', 'datetime_mdy', 'datetime_dmy',
                                'datetime_seconds_ymd', 'datetime_seconds_mdy', 'datetime_seconds_dmy', 'time', 'time_hh_mm_ss');
...
function initFileEnhancements(debug = true) {
