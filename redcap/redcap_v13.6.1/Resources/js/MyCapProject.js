...
$(document).ready(function() {
    ...
    $('[name="template-type"]').click(function(e){
        $.get(app_path_webroot + 'MyCap/participant_info.php?pid='+pid+'&record='+$('#recordVal').val()+'&action=getHTMLByType',
    ...
    $('#saveAnnouncement').submit(function () {
        if (validateAnnouncementInfoForm()) {
            ...
            submitForm(data,app_path_webroot+'MyCapMobileApp/update.php?pid='+pid+'&action=saveAnnouncement',(index == "" ? "AA" : "UA"));
        }
        return false;
    });
...
function publishMyCapVersion() {
    ...
    $.post(app_path_webroot+'MyCapMobileApp/update.php?pid='+pid+'&action=publishVersion', {}, function(data){
        ...
        showProgress(0,0);
        simpleDialogAlt($('div.versionPublishMsg'),1.7);
    });
}
...
function getQRCode(record) {
    ...
    // Get content via ajax
    $.post(app_path_webroot+'MyCap/participant_info.php?pid='+pid+'&record='+record,{ }, function(data) {
        showProgress(0,0);
        if (data == "0") {
            alert(woops);
            return;
 ...
 */
function openEmailTemplatePopup(record, template_type) {
    showProgress(1,0);
    // Id of dialog
    var dlgid = 'messageQR_dialog';
    // Get content via ajax
    $.post(app_path_webroot+'MyCap/participant_info.php?action=getHTML&pid='+pid+'&record='+record+'&type='+template_type,{ }, function(data){
        showProgress(0,0);
        if (data == "0") {
            alert(woops);
            return;
...
function removeAnnouncement(this_announcement_id) {
    var url = app_path_webroot+'MyCapMobileApp/update.php?pid='+pid+'&action=removeAnnouncement';
