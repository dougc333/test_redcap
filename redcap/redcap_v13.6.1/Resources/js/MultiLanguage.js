...
/** @type {string|null} sha1 of the initial state (for dirty check) */
var initialHash = null

/** @type {boolean|null} Tracks whether there are unsaved changes */
var dirty = null

/** @type {string} The current language for translations */
var currentLanguage = ''

/** @type {string|null} The currently shown main tab */
var currentTab = null
...
var itemsChangedSinceTranslated = {}
 ...
 ML.init = function(data) {
    ...
    $(function() {
        ...
        if (config.mode == 'Project') {
            ...
            setInterval(keepAlive, Math.round(hashPollInterval * 60000))
        }
...
function activateTab(tab) {
    ...
    // Fix textareas
    $('div[data-mlm-tab="' + tab + '"] .textarea-autosize').trigger('input')
    setUIEnabledState()
}
...
function activateCategory(cat) {
    ...
    if (cat.startsWith('ui-')) {
        ...
        setTimeout(function() {
            $tab.find('.textarea-autosize').trigger('input')
        }, 5);
        updateUISubheadings()
    }
...
function handleActions(event) {
    ...
    switch (action) {
        ...
        case 'add-system':
            var lang = $source.parents('tr').attr('data-mlm-language') ?? currentLanguage
...
function updateSwitcher(recreate = false) {
    var switcher = $('div.mlm-switcher-buttons')
    if (recreate) {
        switcher.children().remove()
        sortLanguages().forEach(function(langKey) {
            /** @type {Language} */
            var lang = config.settings.langs[langKey]
            var $btn = $('<button data-mlm-action="switch-language" class="btn btn-sm mlm-switcher-button"></button>')
    ...
    switcher.children().each(function() {
        var btn = $(this)
        var langKey = btn.attr('data-mlm-lang')
        btn.removeClass('btn-primary btn-light')
        btn.addClass(langKey == currentLanguage ? 'btn-primary' : 'btn-light')
    })
}
...
//#region ---- Add/Edit/Import Modal

/** @type {AddEditImportModal} The add/edit language modal */
const aeiModal = {
    _$: $(), // The modal
    _modal: null, // bootstrap.Modal object
    _$import: $(), // The import button
    _$importlink: $(), // The import link
    _$file: $(), // The file input
    _$fileradio: $(), // The file radio
    _$sys: $(), // The system lang select
    _$sysradio: $(), // The system lang radio
    _$cancel: $(), // The cancel button
    _$save: $(), // The save button
    _mode: 'add',
    _langKey: '',
...
/**
 * Sets up the Add/Edit/Import modal dialog
 */
function aeiModal_setup() {
    // Store elements
    aeiModal._$ = $('#mlm-add-edit-language-modal')
    aeiModal._$import = $('#mlm-modal-import-button')
    aeiModal._$importlink = $('#mlm-modal-import-link')
    aeiModal._$file = $('#mlm-import-file')
    aeiModal._$fileradio = $('#mlm-import-from-file')
    aeiModal._$sys = $('#mlm-import-system')
    aeiModal._$sysradio = $('#mlm-import-from-system')
    aeiModal._$cancel = $('#mlm-modal-cancel')
    aeiModal._$save = $('#mlm-modal-save')
    aeiModal._modal = new bootstrap.Modal(aeiModal._$[0], { backdrop: 'static' })

    // Hook up some events
    aeiModal._$.on('show.bs.modal', function(e) {
        aeiModal_init()
    })
    aeiModal._$.on('shown.bs.modal', function(e) {
        $('#mlm-add-edit-language-field-key').trigger('focus')
    })
    aeiModal._$cancel.on('click', function(e) {
        aeiModal_cancel()
    })
    aeiModal._$save.on('click', function(e) {
        aeiModal_apply()
    })
    aeiModal._$import.on('click', function(e) {
        aeiModal_startImport()
    })
    aeiModal._$importlink.on('click', function(e) {
        aeiModal_toggleImport(true)
    })
    aeiModal._$file.on('change', function(e) {
        aeiModal._$fileradio.trigger('click')
        aeiModal_readFile(e)
    }) 
    aeiModal._$file.on('keypress input click blur', function(e) {
        aeiModal_updateImportButtonState()
        aeiModal_updateFileWidget()
    }) 
    aeiModal._$sys.on('change', function(e) {
        aeiModal._$sysradio.trigger('click')
        aeiModal_updateImportButtonState()
    })
    aeiModal._$sysradio.on('change', aeiModal_updateImportButtonState)
    aeiModal._$fileradio.on('change', aeiModal_updateImportButtonState)

    $('.custom-file')
        .on('dragover dragenter', function(e) {
            $('.custom-file').addClass('is-dragover');
        })
        .on('dragleave dragend drop', function(e) {
            $('.custom-file').removeClass('is-dragover');
        })
}

/**
 * Initializes the add/edit/import modal before it is shown
 */
function aeiModal_init() {
    $('#mlm-add-edit-language-form').removeClass('was-validated')
    $('#mlm-add-edit-language-form input').removeClass('is-invalid')
    const $saveBtn = $('#mlm-modal-save')
    $saveBtn.children('span.mlm-modal-save-label').text($saveBtn.attr('data-mlm-' + aeiModal._mode) ?? '')
    if (aeiModal._mode == 'edit') {
        aeiModal._$.find('[data-mlm-config]').each(function() {
            const $this = $(this)
            const name = $this.attr('data-mlm-config') ?? ''
            $this.val(config.settings.langs[aeiModal._langKey][name])
        })
    }
    else {
        aeiModal._$.find('[data-mlm-config]').val('')
    }
    $('#mlm-add-edit-language-field-key').trigger('focus')
}

/**
 * Shows the add/edit/import modal
 * @param {string} mode 
 * @param {string} langKey 
 */
function aeiModal_show(mode, langKey = '') {
    log('Opening Add/Edit/Import modal in mode "' + mode + '" for language "' + langKey + '"')
    aeiModal._mode = mode
    aeiModal._langKey = langKey
    aeiModal_toggleImport(false)
    if (mode == 'add') {
        $('.modal-add-title').show()
        $('.modal-edit-title').hide()
        $('.modal-import-options').hide()
        aeiModal._lang = createEmptyLang()
    }
    else if (mode == 'edit') {
        $('#modal-language-key').text(langKey)
        $('.modal-add-title').hide()
        $('.modal-edit-title').show()
        $('.modal-import-options').show()
        aeiModal._lang = copyLang(langKey)
    }
    // @ts-ignore
    aeiModal._modal.show()
}

/**
 * Switches between the add/edit and import views.
 * @param {boolean} visible
 */
function aeiModal_toggleImport(visible) {
    aeiModal._importVisible = visible
    if (visible) {
        $('#mlm-modal-import-button').show()
        $('#mlm-modal-save').hide()
        $('.modal-mlm-edit').hide('fast')
        $('.modal-mlm-import').show('fast')
        $('#mlm-modal-import-link').hide()
        ...
        $('#mlm-modal-import-button').hide()
        $('#mlm-modal-save').show()
        $('.modal-mlm-edit').show('fast')
        $('.modal-mlm-import').hide('fast')
        $('#mlm-modal-import-link').show()
        // Reset file input
        aeiModal._$.find('input[type=file]').val('')
        aeiModal._importedLang = null
        // @ts-ignore
 ...
 */
function aeiModal_updateImportButtonState() {
    const enabled =
        (aeiModal._$fileradio.prop('checked') && aeiModal._importedLang) ||
        (aeiModal._$sysradio.prop('checked') && aeiModal._$sys.val() != '')
    aeiModal._$import.prop('disabled', !enabled)
}

/**
 * Update the state of the file widget
 */
function aeiModal_updateFileWidget() {
    const files = aeiModal._$file.prop('files')
    if (files.length == 0) {
        aeiModal._$.find('.processing-file').addClass('hide')
        aeiModal._$file.removeClass('is-invalid').removeClass('is-valid')
        setTimeout(function() {
            aeiModal._$.find('[data-rc-lang=multilang_34]').html($lang.multilang_34)
        }, 10)
    }
}

/**
 * Switches back from the import view or closes the modal.
 */
function aeiModal_cancel() {
    if (aeiModal._importVisible) {
        aeiModal_toggleImport(false)
    }
    else {
        // @ts-ignore
        aeiModal._modal.hide()
        aeiModal._langKey = ''
        aeiModal._lang = null
    }
}
...
/**
 * Reads a JSON file and checks whether it is a Multi-language feature save file.
 * @param {JQuery.TriggeredEvent} event
 */
function aeiModal_readFile(event) {
    aeiModal._importedLang = null
    aeiModal._$file.removeClass('is-valid')
    const files = aeiModal._$file.prop('files')
    if (files.length === 1) {
        const reader = new FileReader()
        const filename = '' + files[0].name
        const ext = filename.includes('.') ? filename.split('.').reverse()[0].toLowerCase() : 'json' // Try JSON
        aeiModal._$.find('.processing-file').removeClass('hide')
        reader.onload = function(e) {
            if (e.target && e.target.result) {
                processFile(e.target.result.toString(), ext).then(function(data) {
                    aeiModal._importedLang = data
                    if (!aeiModal._importedLang || aeiModal._importedLang.creator !== 'REDCap MLM') {
                        throw 'Not a REDCap Multi-Language Management file'
                    }
                    aeiModal._$file.removeClass('is-invalid').addClass('is-valid')
                    // @ts-ignore
                    event.target.setCustomValidity('')
                }).catch(function(ex) {
                    aeiModal._importedLang = null
                    aeiModal._$file.addClass('is-invalid')
                    // @ts-ignore
    ...
    return new Promise(function(resolve, reject) {
        if (type == 'json') {
            try {
                ...
                var reformattedAlertTranslations = []
                for (var i in lang['alertTranslations']) {
                    var translation = lang['alertTranslations'][i]
                    var reformattedAlertSettings = []
                    var aggregateTemplateDataSetting = {
                        'id': 'alert-sendgrid_template_data'
                    }
                    var templateData = {}
                    for (var j in translation['settings']) {
                        var setting = translation['settings'][j]
                        if (setting['id'].startsWith('alert-sendgrid_template_data')) {
                            var key = setting['id'].split(':')[1] || null
                            if (key) {
 ...
 * Imports a language.
 */
function aeiModal_startImport() {
    const source = aeiModal._$.find('input[name="mlm-import-source"]:checked').val() // 'file' or 'system'
    if (source == 'file') {
        // Perform import
        aeiModal_import()
    }
        ...
        const langKey = aeiModal._$sys.val()
        ajax('get-sys-lang', langKey).then(function(data) {
            aeiModal._importedLang = data.lang
            log('Obtained system language:', data.lang)
            aeiModal_import()
        })
    }
}
...
function aeiModal_import() {
    if (aeiModal._lang == null) {
        error('Import called without a language!')
        return
    }
    //#region Gather modal data
    // What to import
    const importUI = config.mode == 'System' || aeiModal._$.find('input[name="import-include-ui"]').is(":checked")
    const importPSI = config.mode == 'Project' && aeiModal._$.find('input[name="import-include-psi"]').is(":checked")
    // Type of import: 'local' or 'imported'
    const merge = aeiModal._$.find('input[name="import-merge-mode"]:checked').val() 
    const useImported = merge == 'imported'
    const useLocal = merge == 'local'
    // Import empty values - true or false
    const overwrite = aeiModal._$.find('input[name="import-overwrite"]').is(':checked')
    // The import data
    const iLang = aeiModal._importedLang
    /**
    ...
    const localKey = (aeiModal._$.find('input[data-mlm-config=key]').val() ?? '').toString()
    const incomingKey = iLang.key ?? ''
    if (shouldImport(incomingKey, localKey)) {
        aeiModal._$.find('input[data-mlm-config=key]').val(incomingKey)
    }
    const localDisplay = (aeiModal._$.find('input[data-mlm-config=display]').val() ?? '').toString()
    const incomingDisplay = iLang.display ?? ''
    if (shouldImport(incomingDisplay, localDisplay)) {
        aeiModal._$.find('input[data-mlm-config=display]').val(incomingDisplay)
    }
    const localSort = (aeiModal._$.find('input[data-mlm-config=sort]').val() ?? '').toString()
    const incomingSort = iLang.sort ?? ''
    if (shouldImport(incomingSort, localSort)) {
        aeiModal._$.find('input[data-mlm-config=sort]').val(incomingSort)
    }
    const incomingRtl = iLang.hasOwnProperty('rtl') ? (iLang.rtl == true ? 'true' : 'false') : null
    const localRtl = aeiModal._lang.rtl ? 'true' : 'false'
    if (shouldImport(incomingRtl, localRtl)) {
        aeiModal._lang.rtl = iLang.rtl == 'true'
    }
    //#endregion
    //#region Import UI
    // User interface options
    if (importUI && iLang.hasOwnProperty('uiOptions') && Array.isArray(iLang.uiOptions)) {
        for (const item of iLang.uiOptions) {
            // Check if this is a valid id
            if (config.uiMeta[item.id]) {
                if (item.value !== null || overwrite) {
                    aeiModal._lang.ui[item.id] = {
    ...
    if (importUI && iLang.hasOwnProperty('uiTranslations') && Array.isArray(iLang.uiTranslations)) {
        for (const item of iLang.uiTranslations) {
            // Check if this is a valid id
            if (config.uiMeta[item.id]) {
                const translation = item.translation ?? ''
                const hash = item.hash ?? null
                const local = aeiModal._lang.ui[item.id] ?? { translation: '', refHash: null }
    ...
    // Forms
    if (importPSI && iLang.hasOwnProperty('formTranslations') && Array.isArray(iLang.formTranslations)) {
        const itemType = 'form-name'
        const itemName = 'name'
        for (const item of iLang.formTranslations) {
            if (item.hasOwnProperty('id') && item.hasOwnProperty(itemName) && item[itemName].hasOwnProperty('translation')) {
                ...
                const local = get_dd(aeiModal._lang, itemType, name).translation ?? ''
                if (shouldImport(translation, local)) {
                    set_dd(aeiModal._lang, itemType, name, '', translation, hash, false)
                }
            }
            // Form active?
            if (item.hasOwnProperty('id') && item.hasOwnProperty('active') && item.active === true) {
                set_dd(aeiModal._lang, 'form-active', item.id, '', true)
            }
        }
    }
    // Events
    if (importPSI && iLang.hasOwnProperty('eventTranslations') && Array.isArray(iLang.eventTranslations)) {
        for (const item of iLang.eventTranslations) {
            let itemName
            let itemType
            // Name
            itemName = 'name'
    ...
    // Fields
    if (importPSI && iLang.hasOwnProperty('fieldTranslations') && Array.isArray(iLang.fieldTranslations)) {
        for (const field of iLang.fieldTranslations) {
            if (field.hasOwnProperty('id') && config.projMeta.fields.hasOwnProperty(field.id)) {
                ...
                for (const type of Object.keys(typeMeta)) {
                    if (['header','label','note'].includes(type) && field.hasOwnProperty(type)) {
                        ...
                        const local = get_dd(aeiModal._lang, 'field-' + type, field.id).translation ?? ''
                        if (shouldImport(translation, local)) {
                            set_dd(aeiModal._lang, 'field-' + type, field.id, '', translation, hash, false)
                        }
                    }
                }
                // Enum
                if (typeMeta.hasOwnProperty('enum') && field.hasOwnProperty('enum') && Array.isArray(field.enum)) {
                    ...
                    for (const item of field.enum) {
                        if (item.hasOwnProperty('id') && validCodes.includes(item.id.toString()) && item.hasOwnProperty('translation')) {
                            ...
                            const hash = item.hash ?? null
                            const local = get_dd(aeiModal._lang, 'field-enum', field.id, code).translation ?? ''
                            if (shouldImport(translation, local)) {
                                set_dd(aeiModal._lang, 'field-enum', field.id, code, translation, hash, false)
                            }
                        }
                    }
                }
                ...
                if (field.hasOwnProperty('actiontags') && Array.isArray(field.actiontags)) {
                    for (const at of field.actiontags) {
                        ...
                        if (at.hasOwnProperty('translation') && config.projMeta.fields[field.id]['field-actiontag'].hasOwnProperty(id)) {
                            ...
                            const local = get_dd(aeiModal._lang, 'field-actiontag', field.id, id).translation ?? ''
                            if (shouldImport(translation, local)) {
                                set_dd(aeiModal._lang, 'field-actiontag', field.id, id, translation, hash, false)
                            }
                        }
                    }
                }
    ...
    // Matrix Groups
    if (importPSI && iLang.hasOwnProperty('matrixTranslations') && Array.isArray(iLang.matrixTranslations)) {
        for (const matrix of iLang.matrixTranslations) {
            if (matrix.hasOwnProperty('id') && config.projMeta.matrixGroups.hasOwnProperty(matrix.id)) {
                ...
                if (matrix.hasOwnProperty('header')) {
                    ...
                    const local = get_dd(aeiModal._lang, 'matrix-header', name).translation ?? ''
                    if (shouldImport(translation, local)) {
                        set_dd(aeiModal._lang, 'matrix-header', name, '', translation, hash, false)
                    }
                }
                // Enum
                if (matrix.hasOwnProperty('enum') && Array.isArray(matrix.enum)) {
                    ...
                    for (const item of matrix.enum) {
                        if (item.hasOwnProperty('id') && validCodes.includes(item.id.toString()) && item.hasOwnProperty('translation')) {
                            ...
                            const code = item.id.toString()
                            const hash = item.hash ?? null
                            const local = get_dd(aeiModal._lang, 'matrix-enum', name, code).translation ?? ''
                            if (shouldImport(translation, local)) {
                                set_dd(aeiModal._lang, 'matrix-enum', name, code, translation, hash, false)
                            }
                        }
                    }
                }
    ...
    // Survey settings
    if (importPSI && iLang.hasOwnProperty('surveyTranslations') && Array.isArray(iLang.surveyTranslations)) {
        for (const item of iLang.surveyTranslations) {
            if (item.hasOwnProperty('id') && config.projMeta.surveys.hasOwnProperty(item.id) && item.hasOwnProperty('settings') && Array.isArray(item.settings)) {
                ...
                for (const setting of item.settings) {
                    if (typeof setting.id == 'string' && setting.id.startsWith('survey-') && config.projMeta.surveys[name].hasOwnProperty(setting.id) && setting.hasOwnProperty('translation')) {
                        ...
                        const local = get_dd(aeiModal._lang, type, name).translation ?? ''
                        if (shouldImport(translation, local)) {
                            set_dd(aeiModal._lang, type, name, '', translation, hash, false)
                        }
                    }
                }
            }
            // Survey active?
            if (item.hasOwnProperty('id') && item.hasOwnProperty('active') && item.active === true) {
                set_dd(aeiModal._lang, 'survey-active', item.id, '', true)
            }
        }
    }
    // Survey Queue settings
    if (importPSI && iLang.hasOwnProperty('sqTranslations') && Array.isArray(iLang.sqTranslations)) {
        for (const item of iLang.sqTranslations) {
    ...
    // ASIs
    if (importPSI && iLang.hasOwnProperty('asiTranslations') && Array.isArray(iLang.asiTranslations)) {
        for (const item of iLang.asiTranslations) {
                ...
                if (config.projMeta.asis.hasOwnProperty(name)) {
                    for (const setting of item.settings) {
                        if (typeof setting.id == 'string' && setting.id.startsWith('asi-') && config.projMeta.asis[name].hasOwnProperty(setting.id) && setting.hasOwnProperty('translation')) {
                            ...
                            const local = get_dd(aeiModal._lang, type, name).translation ?? ''
                            if (shouldImport(translation, local)) {
                                set_dd(aeiModal._lang, type, form_name, name, translation, hash, false)
                            }
                        }
                    }
                }
    ...
    // Alerts
    if (importPSI && iLang.hasOwnProperty('alertTranslations') && Array.isArray(iLang.alertTranslations)) {
        for (const item of iLang.alertTranslations) {
            if (typeof item.id == 'string' && item.hasOwnProperty('settings') && Array.isArray(item.settings)) {
                        ...
                        if (typeof setting.id == 'string' && setting.id.startsWith('alert-') && config.projMeta.alerts[name].hasOwnProperty(setting.id) && setting.hasOwnProperty('translation')) {
                            ...
                            if (shouldImport(translation, local)) {
                                if (type == 'alert-sendgrid_template_data') {
                                    var templateData = JSON.parse(translation)
                                    for (var key in templateData) {
                                        set_dd(aeiModal._lang, type, name, key, templateData[key], hash, false)
                                    }
                                } else {
                                    set_dd(aeiModal._lang, type, name, '', translation, hash, false)
                                }
    ...
    // Missing Data Codes
    if (importPSI && iLang.hasOwnProperty('mdcTranslations') && Array.isArray(iLang.mdcTranslations)) {
        for (const item of iLang.mdcTranslations) {
    ...
    // PDF Customizations
    if (importPSI && iLang.hasOwnProperty('pdfTranslations') && Array.isArray(iLang.pdfTranslations)) {
        for (const item of iLang.pdfTranslations) {
    ...
    if (importPSI && iLang.hasOwnProperty('protemailTranslations') && Array.isArray(iLang.protemailTranslations)) {
        for (const item of iLang.protemailTranslations) {
    ...
    log("Imported language:", aeiModal._importedLang, aeiModal._lang)
    // Reset import data
    aeiModal._importedLang = null
    aeiModal_toggleImport(false)
}
...
/**
 * Applies the add/edit/import modal.
 */
function aeiModal_apply() {
    if (aeiModal._lang != null) {

        // Validate
        var prevKey = aeiModal._langKey
        /** @type {{ name:string, value:string, valid:boolean}[]} */
        var data = []
        var valid = true
        aeiModal._$.find('[data-mlm-config]').each(function() {
            var $this = $(this)
            var item = { 
                name: $this.attr('data-mlm-config') ?? '', 
                value: ($this.val() ?? '').toString().trim(),
                valid: false
            }
            item.valid = validateLanguage(item.name, item.value, prevKey)
            data.push(item)
            ...
            // @ts-ignore
            this.setCustomValidity(item.valid ? '' : 'Invalid')
        })
        if (!valid) {
            $('#mlm-add-edit-language-form').addClass('was-validated')
            return
        }
        // Copy data
        for (const item of data) {
            aeiModal._lang[item.name] = item.value
        }
        if (aeiModal._mode == 'add') {
            if (Object.keys(config.settings.langs).length == 0) {
                // This is the first, so make it the default one
                config.settings.refLang = aeiModal._lang.key
                config.settings.fallbackLang = aeiModal._lang.key
                // And set it as the current one
                currentLanguage = aeiModal._lang.key
            }
            config.settings.langs[aeiModal._lang.key] = aeiModal._lang
        }
        else if (aeiModal._mode == 'edit') {
            delete config.settings.langs[prevKey]
            config.settings.langs[aeiModal._lang.key] = aeiModal._lang
            // Current or default affected? Fix
            if (config.settings.refLang == prevKey) {
                config.settings.refLang = aeiModal._lang.key
            }
            if (config.settings.fallbackLang == prevKey) {
                config.settings.fallbackLang = aeiModal._lang.key
            }
            if (currentLanguage == prevKey) {
                currentLanguage = aeiModal._lang.key
            }
        }
        renderLanguagesTable()
        ...
        error('Add/Edit: Called apply() without a lang.')
    }
    // @ts-ignore
    aeiModal._modal.hide()
}
...
var delModal = {
    ...
    delete: function(lang) {
        ...
        var langs = Object.keys(config.settings.langs)
...
function generateCSV(data, delimiter) {
    ...
    csv.push('#,"' + data.creator + '"')
    csv.push('#,"' + data.version + '"')
    csv.push('#,"' + data.timestamp + '"')
    csv.push('#,"' + data.instructions + '"')
    // Separating line
    csv.push('#,')
    // Header row
...
function renderFields() {
    ...
    // @ts-ignore
    $container.find('.textarea-autosize').textareaAutoSize()
    // @ts-ignore
    $container.find('[data-toggle="tooltip"]').tooltip()
    // Refresh textarea autosize
    setTimeout(function() {
        $container.find('.textarea-autosize').trigger('input')
    }, 10)
    setUIEnabledState()
...
const rteModal = {
    ...
    _pasteIntoRTE: function(text) {
        // @ts-ignore
        tinyMCE.editors["mlm-rte-editor"].execCommand('mceInsertContent', false, text, true)
    }
}
...
function initializeRichTextEditor(modal, selector, directionality, inverted = false) {
    ...
    const fileimageicons = (((imageuploadIcon != '' || fileuploadIcon != '') ? '| ' : '') + imageuploadIcon + ' ' + fileuploadIcon).trim();
    const config = {
        entity_encoding: "raw",
        ...
        plugins: ['paste autolink image lists link searchreplace code fullscreen table directionality hr'],
        toolbar1: 'formatselect | hr | bold italic underline pre link ' + fileimageicons + ' | alignleft aligncenter alignright alignjustify | undo redo',
...
function getTemplate(name) {
    var $tpl = $($('[data-mlm-template="' + name + '"]').html())
    return $tpl
}
...
function renderSnapshotsTable() {
    ...
    if (config.snapshots === false) {
        ...
        if ($tbody.children().length == 0) {
            ...
            // @ts-ignore
            $tbody.find('[data-toggle="popover"]').popover({ trigger: 'hover', placement: 'right', animation: true })
        }
    }
    setUIEnabledState()
}
        ...
        .then(function(response) {
            ...
            try {
                ...
                $('[data-mlm-tab="usage"] table').on('draw.dt', function() {
                    $('[data-mlm-tab="usage"]').find('[data-toggle="popover"]')['popover']({
                        html: true
                    })
                }).trigger('draw.dt')
            }
...
function processUsageStats(raw, showAllProjects) {
    ...
    for (const entry of raw) {
        if (entry.hasMlm || showAllProjects) {
            data.push({
                projectLink: '<a href="' + window['app_path_webroot'] + 'index.php?pid=' + entry.projectId + '" target="_blank">' + entry.projectTitle + '</a>',
                ...
                mlmLangs: renderLanguagesInfo(entry.nAllLangs, entry.allLangs),
                mlmActive: renderLanguagesInfo(entry.nActiveLangs, entry.activeLangs),
...
function renderProjectId(pid) {
    return '<a href="' + window['app_path_webroot'] + '/ControlCenter/edit_project.php?project=' + pid + '">' + pid + '</a>'
}

function renderProjectStatusIcon(status) {
    switch(status) {
        case 'deleted': 
            return '<span class="fas fa-times" style="color:#C00000;font-size:14px;" aria-hidden="true" title="' + $lang.global_106 + '"></span>'
        case 'completed': 
            return '<span class="fa fa-archive fs11" style="color:#C00000;font-size:14px;" aria-hidden="true" title="' + $lang.edit_project_207 + '"></span>'
        case 'analysis':
            return '<span class="fas fa-minus-circle" style="color:#A00000;font-size:14px;" aria-hidden="true" title="' + $lang.global_159 + '"></span>'
        case 'production':
            return '<span class="far fa-check-square" style="color:#00A000;font-size:14px;" aria-hidden="true" title="' + $lang.global_30 + '"></span>'
        default:
            return '<span class="fas fa-wrench" style="color:#444;font-size:14px;" aria-hidden="true" title="' + $lang.global_29 + '"></span>'
    }
}

function renderProjectLanguage(lang) {
    if (lang == '') return ''
    return '<span class="fas fa-language mlm-stats-with-popover" data-toggle="popover" data-content="' + lang + '" data-trigger="hover"></span>'
}

function renderLanguagesInfo(n, langs) {
    if (n < 1) return '-'
    let lines = []
    ...
    return '<span class="mlm-stats-with-popover" data-toggle="popover" data-content="' + html + '" data-trigger="hover">' + n + '</span>'
}


function renderStatusInfo(item) {
    const debug = item.mlmDebug ? '<i class="fas fa-bug" title="' + $lang.multilang_641 + '"></i>' : ''
    const userDisabled = item.mlmDisabled ? '<i class="fas fa-toggle-off" title="' + $lang.multilang_642 + '"></i>' : ''
    const adminEnabled = item.mlmAdminEnabled ? '<i class="fas fa-user-check text-success" title="' + $lang.multilang_644 + '"></i>' : ''
    const adminDisabled = item.mlmAdminDisabled ? '<i class="fas fa-user-lock text-danger" title="' + $lang.multilang_643 + '"></i>' : ''
...
function checkDirty() {
    // @ts-ignore
    var hash = objectHash(config.settings)
    setDirty(hash !== initialHash)
}
...
/**
 * Performs a server request to keep the MLM session alive
 */
function keepAlive() {
    ajax('keep-alive', null).then(function(data) {
        log('Keep alive signal sent', data)
    })
}
        ...
        for (const type of Object.keys(itemsChangedSinceTranslated[langKey])) {
            for (const name of Object.keys(itemsChangedSinceTranslated[langKey][type])) {
                for (const index of Object.keys(itemsChangedSinceTranslated[langKey][type][name])) {
                    ...
                    const html = (defaultVal != item.default || translation != item.translation) ? '<i class="fas fa-exclamation-circle text-warning" title="' + $lang.multilang_572 + '"></i> ' : '' 
                    ...
                    if (type == 'alert-sendgrid_template_data') {
                        ...
                        if (item.default == '') {
                            ...
                            $item.find('[data-mlm-display="default"]').html('<i class="fas fa-exclamation-circle" title="' + $lang.multilang_600 + '"></i> <b>' + $lang.multilang_600 + '</b>')
 ...
 function validateLanguage(name, val, prevKey) {
    switch(name) {
        ...
        case 'key':
            ...
            var re = new RegExp('^[a-zA-Z](?:[a-zA-Z-]*[a-zA-Z])?$', 'gm')
            var res = re.exec(val)
            var rePass = (res != null) && (res[0] == val)
            var keyPass = (val == prevKey) || !Object.keys(config.settings.langs).map(val => val.toLowerCase()).includes(val.toLowerCase())
