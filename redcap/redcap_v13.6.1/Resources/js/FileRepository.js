...
function fileRepoEditComment(doc_id)
{
	...
	simpleDialog('<div class="fs14">'+lang.docs_1142+' "<b>'+filename+'</b>"'+lang.docs_1143+'</div><div class="mt-3">'+lang.docs_1141+'</div><div class="mt-1"><textarea id="fr-new-comment" class="x-form-field notesbox">'+comment+'</textarea></div>',
...
{
	var filename = $('#file-download-'+doc_id).text();
	simpleDialog('<div class="fs14">'+lang.docs_46+'<div class="mt-3 boldish">'+lang.docs_1148+'</div><div class="mt-3">'+lang.sendit_32+' <code class="fs15 boldish">'+filename+'</code></div></div>', lang.form_renderer_52, null, null, null, lang.global_53, "fileRepoDeleteDo("+doc_id+");", lang.global_19);
}
...
function fileRepoRenameDo(doc_id)
{
	...
	$.post(app_path_webroot + 'index.php?pid=' + pid + '&route=FileRepositoryController:rename', { doc_id: doc_id, folder_id: getParameterByName('folder_id'), name: filename },function(data){
		if (data == '0') {
			...
			simpleDialog(lang.docs_1125, '"'+filename+'" '+lang.docs_1126, null, null, "fileRepoRename("+doc_id+");", );
			return;
		}
		reloadFileRepoTable();
	});
	...
	if (recycle_bin == '1') {
		...
		if (pdf_econsent_system_ip) {
			columns = [
				...
				{data: "5", title: lang.survey_1172}, // IP address
				{data: "6", title: lang.survey_1221},
