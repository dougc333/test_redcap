<template>
    ...
    <DropDown variant="outline-secondary">
        <template #button="{dropdown}">
            <i class="fas fa-cog fa-fw" :class="{'fa-rotate-90': dropdown.isOpen}"></i>
            <span class="ms-2">actions</span>
        </template>
        <template v-slot="{onItemClick, dropdown}" :key="index">
            <DropDownItem>
                <button class="d-flex align-items-center justify-content-between" :disabled="modifyDisabled" @click="onModifyClicked">
                    <i class="fas fa-pencil fa-fw"></i>
                    <span class="ms-2">edit priority</span>
                </button>
            </DropDownItem>
            <DropDownItem>
                <button class="" :disabled="deleteDisabled" @click="onDeleteClicked">
    ...
    <Modal ref="priorityModal">
        ...
        <template v-slot="{onOkClicked}">
            <p class="alert alert-info">Please note that priority will only be updated for messages with a 'waiting' stutus.</p>
...
export default {
    ...
    setup (props, context) {
        ...
        const modifyDisabled = computed( () => {
            const list = messages.value.filter( ({status}) => status===STATUS.WAITING )
            return list.length==0
        } )
        async function onModifyClicked() {
            const totalMessages = messages.value.length
            priorityValue.value = (totalMessages===1) ? messages.value?.[0]?.priority :  ''
        ...
        const deleteDisabled = computed( () => {
            const list = messages.value.filter( ({status}) => status!==STATUS.PROCESSING )
            return list.length==0
        } )

        async function onDeleteClicked() {
            const cardinalityText = messages.value.length===1 ? 'this item' : 'these items'
        ...
        return {
            messages,
            priorityValue,priorityModal,
            modifyDisabled,onModifyClicked,
            deleteDisabled,onDeleteClicked,
        }
    }
}
</script>
