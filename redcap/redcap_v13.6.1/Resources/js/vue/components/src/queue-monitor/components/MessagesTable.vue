<template>
    <div>
        <div class="my-2 d-flex justify-content-between align-items-center">
            <Pagination :totalItems="total" v-model="page" :perPage="perPage" size="sm"/>
            
            <DropDown class="ms-auto" variant="outline-primary">
                <template #button>
                ...
                <template v-slot="{onItemClick, dropdown}" :key="index">
                    <template v-for="(option, index) in perPageOptions" :key="index">
                        <DropDownItem :active="option===perPage" @click="perPage=option">{{option}}</DropDownItem>
                    </template>
                </template>
            </DropDown>
...
export default {
    components: { Table, Modal, Toolbar, Pagination, RefreshButton, DropDown, DropDownItem, },
    setup () {
        ...
        return {
            paginatedMessages,messagesMetadata,
            enabledStatuses,
            
            selectedMessages,selectionIndeterminate,selectAllChecked,
            onSelectAllClicked,

            load,loading,
            
            query,tableFields,
            total,page,perPage,perPageOptions,
            onSort,onQueryInput,
            loadMessages,getStatusIcon,
            getColorByPriority,
            priorityValue,STATUS,
            priorityModal,
        }
    }
}
