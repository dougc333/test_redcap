<template>
    <button class="btn btn-sm btn-outline-primary"  :disabled="groupDisabled" @click="onGroupClicked" :class="classes">
        <slot></slot>
...
const total = computed( () => { return groupMetadata.value?.total })
const totalSelected = computed( () => { return groupMetadata.value?.selected })
const groupDisabled = computed( () => { return (groupMetadata.value?.total ?? 0) <= 0 })
const groups = computed( () => usersStore.groups )
const classes = computed( ()  => {
    return {
        active: groups.value.includes(group.value) ,
        indeterminate: (groupMetadata.value?.selected>0 && groupMetadata.value?.selected!=groupMetadata.value?.total ),
    }
})
