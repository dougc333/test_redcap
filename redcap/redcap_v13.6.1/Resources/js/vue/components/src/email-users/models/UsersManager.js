...
export default class UsersManager {
    ...
    selectGroups(keys) {
        const disableOtherActivityGroups = (key) => {
            for (const activityGroup of activityGroups) {
                if(activityGroup!=key) this._selectedGroups.delete(activityGroup)
            }
        }
    ...
    checkSelectedGroups() {
        for (const [key, group] of this._groups) {
            // const group = this._groups.get(key)
            const intersection = new Set([...group].filter(user => this._selectedUsers.has(user)));
    ...
    toggleSuspended() { this._showSuspended = !this._showSuspended}
    ...
    setPaginatedUsers() {
        let filteredUsers = []
        filteredUsers = this.showSuspended ? [...this._users] : [...this._valid_users]
    ...
    get metadata() {
        const total = this.showSuspended ? this._users.length : this._valid_users.length
    ...
    get totalUsers() { return this.showSuspended ? this._users.length : this._valid_users.length }
    get totalPages() { return Math.ceil(this.totalUsers/this._perPage) }
    get totalFiltered() { return this._filteredUsers.length }
    get state() { return this._state }
    get paginatedUsers() { return this._paginatedUsers }
    get groups() { return Array.from(this._selectedGroups) }
    get selectedUsers() { return Array.from(this._selectedUsers) }
    get selectedIDs() { return Array.from(this._selectedIDs) }
    get filteredUsers() { return Array.from(this._filteredUsers) }
    get showSuspended() { return this._showSuspended }
    get groupsMetadata() { return this._groupsMetadata }
    ...
    get isFilterActive() { return this.groups.length>0 || this._query !== '' }
