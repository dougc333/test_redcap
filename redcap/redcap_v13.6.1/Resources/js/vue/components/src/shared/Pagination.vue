...
export default {
    setup (props, context) {
        ...
        const {modelValue:currentPage, maxVisibleButtons, hideEllipsis} = toRefs(props)
        ...
        const pages = computed( () => {
            ...
            if(currentPage.value>=totalPages.value-delta) start = (totalPages.value-totalButtons+1)
            if(start<1) start = 1
        ...
        function onFirstClicked() { context.emit('update:modelValue', 1) }
        function onLastClicked() { context.emit('update:modelValue', totalPages.value) }
        ...
        return {
            ...
            showEllipsis,
            onPageClicked,
            onFirstClicked,onPrevClicked,onNextClicked,onLastClicked,}
    },
    props: {
        ...
        size: { type: String, default: '', validator(size) {
