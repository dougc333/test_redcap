...
function initTinyMCEglobal(selector, compact, mentionsSettings) {
	...
	if (typeof mentionsSettings == 'undefined') {
		mentionsSettings = { };
	} else {
	...
	var fileimageicons = trim((trim(imageuploadIcon) != '' || trim(fileuploadIcon) != '' ? '| ' : '') + imageuploadIcon + ' ' + fileuploadIcon);
	// Set toolbars
	var toolbar1defaults = 'formatselect | hr | bold italic underline link pre '+fileimageicons+' | alignleft aligncenter alignright alignjustify | undo redo';
	...
	var toolbar1defaultsMobile = 'formatselect | bold italic underline link pre image | fullscreen';
	...
	try {
		tinymce.init({
			...
			plugins: ['paste autolink lists link image searchreplace code fullscreen table directionality hr '+mentionPlugin],
			toolbar1: toolbar1,
			toolbar2: toolbar2,
			...
			mentions: mentionsSettings, // Custom option to use @user mentioning feature
