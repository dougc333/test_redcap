...
function makePanelTitle($title, $id, $collapsed=false) {
	...
	<div style="float:left"><?php print($title) ?></div>
		<div class="opacity65 projMenuToggle" id="<?php print($id) ?>">
			<a href=javascript:;">
				<img src="<?php print(APP_PATH_IMAGES.$imgCollapsed) ?>" class="<?php print($isIE ? 'opacity65' : '') ?>" />
			</a>
		</div>
	<?php
...
//Data Logging
if ($user_rights['data_logging'] || $user_rights['user_rights']) {
	...
	if ($GLOBALS['email_logging_enable_global'] && $user_rights['user_rights'] && $user_rights['data_logging']) {
		$appsMenu .= RCView::span(array('style'=>'color:#777;margin:0 7px 0 6px;'), $lang['global_43']);
	}
	if ($GLOBALS['email_logging_enable_global'] && $user_rights['user_rights']) {
	...
	$makeBody = function() use($project_id, $lang) {
		...
		<div class="menubox">
			...
			<span class="d-block">
				...
				<a href="<?php print(APP_PATH_WEBROOT.'ehr.php?standalone_launch=1') ?>"><?php echo $lang['control_center_4893'] ?></a>
			</span>

			<?php if($fhirUser->can_use_mapping_helper) : ?>
			<span class="d-block">
				<i class="fas fa-code-branch"></i>
				<a href="<?php print(FhirMappingHelper::getLink($project_id)) ?>"><?php echo $lang['control_center_4894'] ?></a>
			</span>
			<?php endif; ?>
			...
			<div class="access-token-wrapper mt-1" data-accessToken="<?= $accessToken ?>" >
				...
				<?php if($accessToken) : ?>
					...
					<span><?php echo $lang['control_center_4895'] ?></span>
				<?php else : ?>
					<i class="fas fa-times-circle text-danger"></i>
					<span><?php echo $lang['control_center_4896'] ?></span>
				<?php endif; ?>
				</span>
			</div>
			...
			<div class="auto-login-wrapper">
				...
				<?php if($mappedEhrUser) : ?>
					...
					<span title="automatic log-in enabled during a launch from EHR with the user '<?= $mappedEhrUser ?>'"><?php echo $lang['control_center_4897'] ?></span>
				<?php else : ?>
					<i class="fas fa-times-circle text-danger"></i>
					<span title="no EHR user is mapped to your REDCap user"><?php echo $lang['control_center_4898'] ?></span>
				<?php endif; ?>
				</span>
			</div>
...
<div class="container-fluid mainwindow">
	...
	<div id="mainwindowsub" style="flex-wrap:nowrap;-ms-flex-wrap:nowrap;" class="row row-offcanvas row-offcanvas-left <?php echo (isset($_SESSION["mc_open"]) && $_SESSION["mc_open"] == '1' && $user_messaging_enabled == 1 ? 'body-override' : '') ?>" data_open="0">
        <!-- left-hand menu -->
...
<script type="module">
	import {ParcelBadge} from '<?=APP_PATH_JS?>vue/components/dist/lib.es.js'
