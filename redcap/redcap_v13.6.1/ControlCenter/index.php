...
$lastUpgradeDate = DateTimeRC::format_ts_from_ymd(System::getLastUpgradeDate());
...
if (isset($read_replica_hostname) && $read_replica_hostname != '' && isset($read_replica_username) && isset($read_replica_password) && isset($read_replica_db) && !db_connect(false, false, true, true))
{
	print "<div class='red mb-2'><i class='fa fa-exclamation-circle'></i> ".$lang['check_174']."</div>";
}
...
?>
    <div class="gray2 my-3">
        <div class="ps-1 pb-1 font-weight-bold"><?=$lang['control_center_4859']?></div>
        <div class="ps-1 pb-1"><i class="fa-regular fa-calendar-check" style="margin:0 2px 0 1px;"></i> <?=$lang['control_center_4832']?><code style="font-size:100%;margin-left:5px;"><?=$lastUpgradeDate?></code></div>
        <div class="ps-1 pb-1"><i class="fa-solid fa-folder-tree"></i> <?=$lang['control_center_4858']?><code style="margin-left:5px;"><?=$web_ini?></code></div>
		<?php if(!empty($ini_mismatch)): ?>
        <div class="py-1 ml-4 mb-1 yellow fs12" style="background-color:#fff7d242;"><?=$ini_mismatch?></div>
		<?php endif; ?>
        <div class="ps-1 pb-1"><i class="fa-solid fa-folder-tree"></i> <?=$lang['control_center_4860']?><code style="margin-left:5px;"><?=ini_get('error_log')?></code></div>
    </div>
<?php
// Display alert message in Control Center if any new REDCap versions are available
