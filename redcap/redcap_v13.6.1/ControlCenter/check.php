...
{
	print  "<div class='yellow'>
				<img src='".APP_PATH_IMAGES."exclamation_orange.png'>".$lang['check_103']."	</div>";
}
...
if (isset($read_replica_hostname) && $read_replica_hostname != '' && isset($read_replica_username) && isset($read_replica_password) && isset($read_replica_db))
{
	...
	if (db_connect(false, false, true, true, true)) {
		...
		if ($replicaLagTime === false) {
			...
			// Lag is too high
			print "<div class='yellow'><img src='".APP_PATH_IMAGES."exclamation_orange.png'> ".$lang['check_175']." $replicaLagTime ".$lang['check_176']." ".System::REPLICA_LAG_MAX." ".$lang['check_177']."</div>";
		} else {
			// Lag is fine
		...
		print "<div class='red'><img src='".APP_PATH_IMAGES."exclamation.png'> ".$lang['check_174']."</div>";
	}
}
...
if ($edoc_storage_option == '0' || $edoc_storage_option == '3') {
	...
	if (!$wdc->open()) {
		// Send error response
		print "<div class='red'><img src='".APP_PATH_IMAGES."exclamation.png'> ".$lang['check_136']."
				<b>".dirname(APP_PATH_DOCROOT).DS."webtools2".DS."webdav".DS."webdav_connection.php</b>. ".$lang['check_137']." \"$webdav_hostname\" ".$lang['check_138']." \"$webdav_path\", ".$lang['check_139']."</div>";
	}
	...
	if ($http_status == '201') {
		// Success
		print "<div class='darkgreen'><img src='".APP_PATH_IMAGES."tick.png'>
				<b style='color:green;'>".$lang['check_140']." \"$webdav_hostname\" ".$lang['check_138']." \"$webdav_path\"</b></div>";
		// Now delete the file we just created
		$http_status = $wdc->delete($webdav_path . $test_file_name);
	} else {
		// Failed
		print "<div class='red'><img src='".APP_PATH_IMAGES."exclamation.png'> ".$lang['check_140']."
				<b>".dirname(APP_PATH_DOCROOT).DS."webtools2".DS."webdav".DS."webdav_connection.php</b>. ".$lang['check_137']." \"$webdav_hostname\" ".$lang['check_138']." \"$webdav_path\", ".$lang['check_142']."</div>";
	}
	$wdc->close();
}
