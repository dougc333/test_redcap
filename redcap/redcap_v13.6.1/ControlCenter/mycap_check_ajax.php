...
if (isset($_GET['action']) && $_GET['action'] == 'listParticipants') {
    ...
    if($sel_proj_id > 0) {
        $sql = "SELECT code, record FROM redcap_mycap_participants WHERE project_id = '".$sel_proj_id."' AND is_deleted = 0 ORDER BY record";
        $q = db_query($sql);
        ...
        while ($row = db_fetch_assoc($q))
        {
            ...
            $identifier = Participant::getParticipantIdentifier($row['record'], $sel_proj_id);
