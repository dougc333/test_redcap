	...
	<tr>
		...
		<td>
			<input type='text' class='x-form-text x-form-field' id='username' name='username' maxlength='255' <?php echo (empty($user_obj->ui_id) ? '' : 'style="background-color:#ddd;"') ?>
				onblur="if (this.value.length > 0) {if(!chk_username(this,<?php print ($auth_meth_global == 'ldap' || $auth_meth_global == 'aaf' || $auth_meth_global == 'aaf_table' || $auth_meth_global == 'ldap_table') ? "true" : "false" ?>)) return alertbad(this,'<?php echo $lang['rights_438'] ?>'); }"
