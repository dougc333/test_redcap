...
function db_connect($reportOnlySuperUserErrors=false, $useUpgradesUser=false, $createReadReplicaConnection=false, $forceReadReplicaConnection=false, $bypassReadReplicaLagCheck=false)
{
	...
	if (isset($read_replica_hostname) && $read_replica_hostname != '' && isset($read_replica_username) && isset($read_replica_password) && isset($read_replica_db)) {
	...
	if (!$useUpgradesUser && isset($conn) && is_object($conn) && !($readReplicaIsAvailable && $createReadReplicaConnection)) {
		...
		if ($readReplicaIsAvailable && $createReadReplicaConnection) {
            ...
            // If lag time is too high, do not use replica connection
            if (!$bypassReadReplicaLagCheck && System::readReplicaLagTimeIsTooHigh()) {
	...
	if ($readReplicaIsAvailable) {
		if ($createReadReplicaConnection) {
...
function db_query($sql, $params=[], $conn = null, $resultmode = MYSQLI_STORE_RESULT, $forceUsePrimaryDbConnection = false)
{
						 ...
						 $user=null, $participant_id=null, $form=null)
{
	...
	if ($element_enum !== null && strtolower(substr(trim($element_enum), 0, 7)) == "select ")
	{
...
function replaceUrlOldRedcapVersion($val)
{
    ...
    if (strpos($val, "/redcap_v") !== false) {
        preg_match_all("/(\/)(redcap_v)([0-9]{1,2}\.[0-9]{1,2}\.[0-9]{1,2})(\/)/", $val, $matches);
        if (isset($matches[0])) {
            $versionCurent = "/redcap_v" . REDCAP_VERSION . "/";
            foreach ($matches[0] as $key=>$fullmatch) {
                $versionInUrl = $matches[3][$key];
                if ($versionInUrl != REDCAP_VERSION) {
                    $val = str_replace($fullmatch, $versionCurent, $val);
                }
            }
        }
    }
...
function renderJsVars()
{
	...
	$(function(){ $('.redcap-chart-colorblind-toggle').removeClass('invisible').find('a').html(<?=(isset($_COOKIE['redcap_colorblind']) && $_COOKIE['redcap_colorblind'] == '1') ? 'window.lang.dash_73' : 'window.lang.dash_72' ?>).click(function(){ toggleChartColorBlind(); } ); });
	<?php } ?>
	// System values
...
function datediff($d1="", $d2="", $unit=null, $returnSigned=false, $returnSigned2=false)
{
	...
	$dat1 = mktime(0,0,0,$dt1[1],$dt1[2],(int)$dt1[0]) + $d1sec;
	$dat2 = mktime(0,0,0,$dt2[1],$dt2[2],(int)$dt2[0]) + $d2sec;
